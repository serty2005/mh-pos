...
func (r *Repository) CreateRole(ctx context.Context, v domain.Role) (domain.Role, error) {
	return scanRole(r.pool.QueryRow(ctx, `
INSERT INTO cloud_roles(id,restaurant_id,name,permissions_json,active,created_at,updated_at)
VALUES ($1,$2,$3,$4::jsonb,$5,$6,$7)
RETURNING id,restaurant_id,name,permissions_json::text,active,created_at,updated_at`,
		v.ID, v.RestaurantID, v.Name, v.PermissionsJSON, v.Active, v.CreatedAt, v.UpdatedAt))
}

func (r *Repository) GetRole(ctx context.Context, id string) (domain.Role, error) {
	v, err := scanRole(r.pool.QueryRow(ctx, `SELECT id,restaurant_id,name,permissions_json::text,active,created_at,updated_at FROM cloud_roles WHERE id = $1`, strings.TrimSpace(id)))
	return v, normalizeErr(err)
}

func (r *Repository) ListRoles(ctx context.Context, restaurantID string) ([]domain.Role, error) {
	rows, err := r.pool.Query(ctx, `SELECT id,restaurant_id,name,permissions_json::text,active,created_at,updated_at FROM cloud_roles WHERE restaurant_id = $1 ORDER BY id`, strings.TrimSpace(restaurantID))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *Repository) CreateEmployee(ctx context.Context, v domain.Employee) (domain.Employee, error) {
	return scanEmployee(r.pool.QueryRow(ctx, `
INSERT INTO cloud_employees(id,restaurant_id,role_id,name,status,pin_hash,pin_credential_version,permission_snapshot_json,suspended_at,archived_at,created_at,updated_at)
VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb,$9,$10,$11,$12)
RETURNING id,restaurant_id,role_id,name,status,pin_hash,pin_credential_version,permission_snapshot_json::text,suspended_at,archived_at,created_at,updated_at`,
		v.ID, v.RestaurantID, v.RoleID, v.Name, v.Status, v.PINHash, v.PINCredentialVersion, v.PermissionSnapshotJSON, v.SuspendedAt, v.ArchivedAt, v.CreatedAt, v.UpdatedAt))
}
...
SET role_id=$2,name=$3,status=$4,pin_hash=$5,pin_credential_version=$6,permission_snapshot_json=$7::jsonb,suspended_at=$8,archived_at=$9,updated_at=$10
WHERE id=$1
RETURNING id,restaurant_id,role_id,name,status,pin_hash,pin_credential_version,permission_snapshot_json::text,suspended_at,archived_at,created_at,updated_at`,
		v.ID, v.RoleID, v.Name, v.Status, v.PINHash, v.PINCredentialVersion, v.PermissionSnapshotJSON, statusTime(v.Status, domain.EmployeeSuspended, v.SuspendedAt, v.UpdatedAt), v.ArchivedAt, v.UpdatedAt))
	return out, normalizeErr(err)
}

func (r *Repository) GetEmployee(ctx context.Context, id string) (domain.Employee, error) {
	v, err := scanEmployee(r.pool.QueryRow(ctx, `SELECT id,restaurant_id,role_id,name,status,pin_hash,pin_credential_version,permission_snapshot_json::text,suspended_at,archived_at,created_at,updated_at FROM cloud_employees WHERE id = $1`, strings.TrimSpace(id)))
	return v, normalizeErr(err)
}

func (r *Repository) ListEmployees(ctx context.Context, restaurantID string) ([]domain.Employee, error) {
	rows, err := r.pool.Query(ctx, `SELECT id,restaurant_id,role_id,name,status,pin_hash,pin_credential_version,permission_snapshot_json::text,suspended_at,archived_at,created_at,updated_at FROM cloud_employees WHERE restaurant_id = $1 ORDER BY id`, strings.TrimSpace(restaurantID))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
INSERT INTO cloud_catalog_items(id,restaurant_id,kind,name,sku,base_unit,status,created_at,updated_at)
VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
RETURNING id,restaurant_id,kind,name,sku,base_unit,status,created_at,updated_at`,
		v.ID, v.RestaurantID, v.Kind, v.Name, v.SKU, v.BaseUnit, v.Status, v.CreatedAt, v.UpdatedAt))
	if err != nil {
		return domain.CatalogItem{}, err
	}
...
SET kind=$2,name=$3,sku=$4,base_unit=$5,status=$6,updated_at=$7
WHERE id=$1
RETURNING id,restaurant_id,kind,name,sku,base_unit,status,created_at,updated_at`,
		v.ID, v.Kind, v.Name, v.SKU, v.BaseUnit, v.Status, v.UpdatedAt))
	if err != nil {
...
func (r *Repository) GetCatalogItem(ctx context.Context, id string) (domain.CatalogItem, error) {
	v, err := scanCatalogItem(r.pool.QueryRow(ctx, `SELECT id,restaurant_id,kind,name,sku,base_unit,status,created_at,updated_at FROM cloud_catalog_items WHERE id = $1`, strings.TrimSpace(id)))
	return v, normalizeErr(err)
}

func (r *Repository) ListCatalogItems(ctx context.Context, restaurantID string) ([]domain.CatalogItem, error) {
	rows, err := r.pool.Query(ctx, `SELECT id,restaurant_id,kind,name,sku,base_unit,status,created_at,updated_at FROM cloud_catalog_items WHERE restaurant_id = $1 ORDER BY id`, strings.TrimSpace(restaurantID))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *Repository) CreateMenuItem(ctx context.Context, v domain.MenuItem) (domain.MenuItem, error) {
	return scanMenuItem(r.pool.QueryRow(ctx, `
INSERT INTO cloud_menu_items(id,restaurant_id,catalog_item_id,category_id,name,price,currency,status,availability_json,station_routing_key,created_at,updated_at)
VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10,$11,$12)
RETURNING id,restaurant_id,catalog_item_id,COALESCE(category_id,''),name,price,currency,status,availability_json::text,COALESCE(station_routing_key,''),created_at,updated_at`,
		v.ID, v.RestaurantID, v.CatalogItemID, nullableText(v.CategoryID), v.Name, v.Price, v.Currency, v.Status, v.AvailabilityJSON, nullableText(v.StationRoutingKey), v.CreatedAt, v.UpdatedAt))
}
...
SET catalog_item_id=$2,category_id=$3,name=$4,price=$5,currency=$6,status=$7,availability_json=$8::jsonb,station_routing_key=$9,updated_at=$10
WHERE id=$1
RETURNING id,restaurant_id,catalog_item_id,COALESCE(category_id,''),name,price,currency,status,availability_json::text,COALESCE(station_routing_key,''),created_at,updated_at`,
		v.ID, v.CatalogItemID, nullableText(v.CategoryID), v.Name, v.Price, v.Currency, v.Status, v.AvailabilityJSON, nullableText(v.StationRoutingKey), v.UpdatedAt))
	return out, normalizeErr(err)
}

func (r *Repository) GetMenuItem(ctx context.Context, id string) (domain.MenuItem, error) {
	v, err := scanMenuItem(r.pool.QueryRow(ctx, `SELECT id,restaurant_id,catalog_item_id,COALESCE(category_id,''),name,price,currency,status,availability_json::text,COALESCE(station_routing_key,''),created_at,updated_at FROM cloud_menu_items WHERE id = $1`, strings.TrimSpace(id)))
	return v, normalizeErr(err)
}

func (r *Repository) ListMenuItems(ctx context.Context, restaurantID string) ([]domain.MenuItem, error) {
	rows, err := r.pool.Query(ctx, `SELECT id,restaurant_id,catalog_item_id,COALESCE(category_id,''),name,price,currency,status,availability_json::text,COALESCE(station_routing_key,''),created_at,updated_at FROM cloud_menu_items WHERE restaurant_id = $1 ORDER BY id`, strings.TrimSpace(restaurantID))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func upsertKindFoundation(ctx context.Context, tx pgx.Tx, v domain.CatalogItem) error {
	...
	case domain.CatalogItemDish:
		...
		return err
	case domain.CatalogItemGood, domain.CatalogItemRawMaterial:
...
func scanRole(row scanner) (domain.Role, error) {
	var v domain.Role
	err := row.Scan(&v.ID, &v.RestaurantID, &v.Name, &v.PermissionsJSON, &v.Active, &v.CreatedAt, &v.UpdatedAt)
	return v, err
}
...
func scanEmployee(row scanner) (domain.Employee, error) {
	...
	var status string
	err := row.Scan(&v.ID, &v.RestaurantID, &v.RoleID, &v.Name, &status, &v.PINHash, &v.PINCredentialVersion, &v.PermissionSnapshotJSON, &v.SuspendedAt, &v.ArchivedAt, &v.CreatedAt, &v.UpdatedAt)
...
func scanCatalogItem(row scanner) (domain.CatalogItem, error) {
	...
	err := row.Scan(&v.ID, &v.RestaurantID, &kind, &v.Name, &v.SKU, &v.BaseUnit, &status, &v.CreatedAt, &v.UpdatedAt)
...
func scanMenuItem(row scanner) (domain.MenuItem, error) {
	...
	var status string
	err := row.Scan(&v.ID, &v.RestaurantID, &v.CatalogItemID, &v.CategoryID, &v.Name, &v.Price, &v.Currency, &status, &v.AvailabilityJSON, &v.StationRoutingKey, &v.CreatedAt, &v.UpdatedAt)
	v.Status = domain.LifecycleStatus(status)
	return v, err
}
