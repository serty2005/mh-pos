...
func RequiredSchema() []platformpg.SchemaRequirement {
	return []platformpg.SchemaRequirement{
		...
		},
		{
			...
			MigrationFile: "001_init.sql",
			Columns:       []string{"id", "name", "permissions_json", "active", "cloud_version", "archived_at", "created_at", "updated_at"},
		},
		{
			Table:         "cloud_employees",
			RequiredBy:    "cloud master-data authority employee lifecycle and PIN credential storage",
			MigrationFile: "001_init.sql",
			Columns:       []string{"id", "role_id", "name", "status", "pin_hash", "pin_credential_version", "permission_snapshot_json", "cloud_version", "suspended_at", "archived_at", "created_at", "updated_at"},
			Indexes:       []string{"cloud_employees_status"},
		},
		{
			Table:         "cloud_employee_restaurant_memberships",
			RequiredBy:    "authoritative employee restaurant scope",
			MigrationFile: "001_init.sql",
			Columns:       []string{"employee_id", "restaurant_id", "created_at", "updated_at"},
			Indexes:       []string{"cloud_employee_memberships_restaurant"},
		},
		{
