...
Статус: актуальная context map для текущего cashier runtime и целевого полного пилота.
...
| `Inventory` | Cloud-owned stock documents, stock ledger, costing, balances, stop-list authority, inventory worker policy | реализовано сейчас: Cloud-centric Event-Driven Inventory для normalized item payloads, bounded retro recalculation job/DAG lifecycle и первый ClickHouse `olap_stock_moves` read; Edge-side manual stock foundation был pre-pilot legacy и удален | semi-finished auto-production split, richer costing math, production-grade balance rebuild, aggregate OLAP reads | ERP/accounting integration | Edge-side stock documents/moves, order/payment/check direct mutation |
| `Production` | KDS tickets, stations, cooking/dish served lifecycle, `KitchenTicketStatusChanged`, `ItemServed`, `ProductionCompleted` input events | реализовано сейчас для минимального POS Edge ticket lifecycle и `KitchenTicketStatusChanged`/`ItemServed`; расширенный production/receipt/proposal flow запланирован далее | chef receipt/catalog/recipe proposal flows | hardware bump-bar/printer integrations, rich BI dashboards | financial close and Cloud ledger writes |
...
| `Accounting / Finance / Analytics` | reporting, reconciliation, profit views, immutable business event archive, OLAP API | реализована основа in Cloud projections: event-type stats, coarse shift finance counters, detailed current financial operation projection, ClickHouse `raw_business_events` и bounded `olap_stock_moves` API | aggregate OLAP reads, backfill controls and costing-dependent BI | ERP/accounting integration and rich BI UI | runtime capture/check mutation |
...
- Просмотр закрытых заказов относится к POS local read model: bounded pagination/filtering реализовано сейчас. Manifest-only export-plan, export-only archive readiness и destructive apply/VACUUM для старых closed orders относятся к operations/data lifecycle и не меняют cashier runtime ownership.
...
Замороженный принцип для immutable event archive:
...
- реализовано сейчас: `stock_ledger.unit_cost_minor` stores event-time fallback cost; bounded DAG-based retro recalculation пересчитывает только costing fields/status existing ledger rows; richer costing math и production-grade balance rebuild запланированы далее;
