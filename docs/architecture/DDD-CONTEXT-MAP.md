...
| `Inventory` | Cloud-owned stock documents, stock ledger, costing, balances, stop-list authority, inventory worker policy | реализовано сейчас: Cloud-centric Event-Driven Inventory для normalized item payloads и первый ClickHouse `olap_stock_moves` read; Edge-side manual stock foundation был pre-pilot legacy и удален | full recipe/costing implementation, stop-list sync, retro costing, aggregate OLAP reads | ERP/accounting integration | Edge-side stock documents/moves, order/payment/check direct mutation |
