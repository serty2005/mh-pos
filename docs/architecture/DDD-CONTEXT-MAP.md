...
| `Financial Operations` | append-only cancellation/refund ledger, operation item scopes, inventory disposition, no-over-compensation rules | реализовано сейчас: full/partial cancellation and refund records, full-check cashier UI actions, `CancellationRecorded`, `RefundRecorded` | rich partial UI for line/quantity/modifier/service/tip only if pilot acceptance requires it | richer accounting projections | inventory mutation, PSP refund execution, fiscal correction documents |
