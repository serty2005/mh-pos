...
- `GET /api/v1/storage/status`, `POST /api/v1/storage/retention/dry-run`, `POST /api/v1/storage/archive/export-plan` и `POST /api/v1/storage/archive/export` являются локальными POS operational lifecycle API. Они не создают sync envelopes; dry-run, manifest-only export-plan и export-only archive только сообщают active/open blockers и non-sent `edge_to_cloud` outbox rows как blocking state для будущей destructive retention/archive policy. Export-only manifest явно возвращает `runtime_rows_deleted = false`.
