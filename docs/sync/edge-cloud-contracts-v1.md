...
Запланировано до полного пилота для остальных inventory/proposal payloads, необходимых полному Cloud Inventory Engine и ClickHouse OLAP: Cloud review/apply для `CatalogItemChangeSuggested`/`RecipeChangeSuggested`, расширенные `RefundRecorded`/`CancellationRecorded` inventory effects и OLAP-проекции поверх `raw_business_events`. Все payloads передаются внутри стандартного sync envelope в `payload.data`.
