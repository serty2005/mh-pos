...
Реализовано сейчас: Cloud Edge-ready snapshot endpoint:
...
- Cloud master-data authority создает versioned publications и обновляет `cloud_master_data_packages` для `restaurants`, `staff`, `catalog`, `menu`.
