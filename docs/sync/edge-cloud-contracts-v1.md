...
- Реализовано сейчас: `catalog_items` являются tenant-level identity. `restaurant_id` у catalog item/folder/tag/item-tag link может быть пустым для tenant-owned row; `catalog_item_id` не должен заменяться menu text, SKU или category text в downstream contracts.
...
- Реализовано сейчас: `menu_items` являются restaurant-effective projection поверх tenant catalog. Каждый menu item сохраняет собственный `menu_item_id`, обязательный `catalog_item_id` и restaurant-specific overrides: `name`, `price`, `category_id` как stable menu folder/ticket category identity, `tag_id`, `tax_profile_id`, `availability_json`, lifecycle `active` и `runtime_status` (`available`, `unavailable`, `hidden`). Edge получает только `menu_items` выбранного ресторана; изменение menu item одного ресторана не меняет menu item другого ресторана, даже если оба ссылаются на один `catalog_item_id`.
...
- `folder_parameters[]` остаются restaurant-scoped и содержат `restaurant_id`; `folders[]`, `tags[]` и `item_tags[]` могут быть tenant-level rows с пустым `restaurant_id`.
...
Cloud Inventory Worker
  -> PostgreSQL stock_ledger
  -> Async Stock Moves Forwarder
...
- Shift finance projection не является полной ledger projection для cancellation/refund; detailed reporting foundation читает `cloud_projection_financial_operations` для current `CancellationRecorded`/`RefundRecorded`. `GET /api/v1/reporting/financial-operations?restaurant_id=&business_date_from=&business_date_to=&operation_type=&shift_id=&original_shift_id=&check_id=&limit=&offset=` реализовано сейчас как bounded read-only Cloud reporting API без raw sync payload. Активный `cloud-ui-g` reporting screen еще не реализует.
...
`CatalogItemChangeSuggested` и `RecipeChangeSuggested` создаются POS Edge proposal routes и далее попадают в Cloud review/apply flow с таблицами `cloud_catalog_suggestions`/`cloud_recipe_suggestions` и feedback stream `proposal_feedback`:
