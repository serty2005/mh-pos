...
- `pos_cashier_runtime`: переиспользует summary из `cloud_to_edge_masterdata` или `scripts/.local-masterdata-summary.json` и проверяет backend path `login -> personal shift -> cash shift -> hall/table/menu reads -> order -> regular/modifier/service lines -> precheck -> payment by precheck_id -> final check -> closed orders -> check get/reprint -> cancellation ledger в той же смене -> financial operations -> storage status -> archive export/verify -> apply-readiness -> destructive apply + VACUUM -> closed-orders absence -> archive read-plan/lookup preview`.
- Контракт smoke helper теперь проверяет полный Storage Retention Lifecycle: verified JSONL archive, `ready_for_destructive_apply = true`, `runtime_rows_deleted = true`, отсутствие удаленного чека в active `/orders/closed` и доступность preview/lookup из archive artifact.
