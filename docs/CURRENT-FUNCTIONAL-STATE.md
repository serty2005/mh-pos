...
- `ProductionCompleted` реализовано сейчас как `PRODUCTION/IN` для готовой/полуготовой позиции и, при наличии active recipe, ingredient `OUT`; при отсутствии recipe/cost basis worker сохраняет факт production с `estimated`/`needs_recalculation`, не выполняя retro recalculation DAG.
