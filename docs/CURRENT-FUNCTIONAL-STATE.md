...
- Автоматическая Cloud -> Edge доставка без manual publish реализована сейчас: package rows создаются после Edge assignment/pairing и обновляются после подтвержденных Cloud master-data commits для назначенных Edge. Persistent per-Edge delivery state хранит Cloud/ACK version, lag, last sync, safe error и retry; неизменный replay не создает новую publication version, а failed assembly повторяется на следующем exchange. Physical ESC/POS printing, Telegram reports и sales dashboard пока не реализованы. External License Server, versioned entitlement snapshots и backend gates для существующих table/kitchen/warehouse surfaces реализованы сейчас; будущие telegram/waiter/checker runtime получают gates вместе со своими routes/workers. Основной cashier flow является базовым нелицензируемым runtime; `waiter-space` не должен закрывать shared order/precheck/payment backend, пока нет отдельного backend-discriminated waiter surface. Tenant-level roles/employees, employee restaurant memberships, `organization.manage`, tenant catalog identity и restaurant menu overrides реализованы сейчас. QR-enabled service items (POS-52): `qr_confirmation_enabled`, auto-derived `single_unit_per_line`, и `validity_mode` (`cash_session`/`business_date`/`absolute_date`) реализованы сейчас в Cloud catalog, EdgeCatalogItem delivery, POS Edge order validation (qty > 1 блокируется backend) и Cloud UI форме. Ticket issuance (POS-48) реализовано сейчас: после полной оплаты и закрытия final check для каждой QR-enabled line транзакционно создается одна `ticket_units` единица с immutable name/sale date/timezone/validity, уникальным UUIDv7 ticket number, порядковым номером внутри кассовой смены и QR payload `MHT1:<ticket_number>` без secrets; replay не создает второй билет, reprint (`POST /api/v1/tickets/{id}/reprint`) использует тот же ticket number и QR с COPY marker без новой единицы, а `TicketIssued` синхронизируется Edge -> Cloud как accepted audit (operational sales projection/API — POS-40). QR checker lookup/confirm/revoke — в post-deploy цикл. Reprint возвращает snapshot без printer orchestration. Целевой контракт описан в `docs/project-management/EXHIBITION-ALPHA-PILOT-REQUIREMENTS.md`.
...
- До первого выставочного запуска: автоматическая per-Edge batch assembly без Publish action, физическая QR/ticket печать и ESC/POS subsystem (POS-64), Cloud sales dashboard/API (POS-40) и Telegram worker. Ticket issuance, уникальный номер, validity snapshot и QR payload реализованы сейчас (POS-48). Внешний licensing authority и gates существующих table/kitchen/warehouse surfaces реализованы сейчас; tenant-level roles/employees, memberships, tenant catalog identity и restaurant menu overrides также реализованы сейчас.
