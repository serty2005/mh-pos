...
- Лицензионная цель для дальнейших smoke: Edge -> Cloud batch должен содержать только module-owned события включенных модулей по ownership из `docs/backend/LICENSE-ENTITLEMENTS.md`; базовые cashier financial facts остаются синхронизируемым ядром подключенного Cloud-контура.
