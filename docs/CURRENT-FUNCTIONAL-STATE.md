...
Статус: реализовано сейчас по коду, тестам, Plane-аудиту и документации на 2026-06-30; цели полного и выставочного альфа-пилотов зафиксированы отдельно и не считаются текущим runtime.
...
- Подтвержденного runtime для настоящего платежного процессинга, фискального адаптера, COGS/margin OLAP reads, production BI, production-grade kitchen timing API и расширенных cooking events за пределами ticket lifecycle foundation.
- Автоматическая Cloud -> Edge доставка без manual publish реализована сейчас: package rows создаются после Edge assignment/pairing и обновляются после подтвержденных Cloud master-data commits для назначенных Edge. Persistent per-Edge delivery state хранит Cloud/ACK version, lag, last sync, safe error и retry; неизменный replay не создает новую publication version, а failed assembly повторяется на следующем exchange. Physical ESC/POS printing для нефискальных `precheck` и `ticket` реализована сейчас через Edge print queue/worker; clean-stack прогон 2026-06-28 отправил `precheck` и `ticket` на TCP-принтер `10.25.1.201:9100`, оба job завершились `succeeded`, оператор подтвердил выход двух чеков. ESC/POS/SVG renderer исправлен после съезда крупного `{f:double}` текста на 48CPL: строки двойной ширины используют эффективную ширину `CPL/2`; требуется повторное визуальное подтверждение ticket. Telegram reports и sales dashboard пока не реализованы. External License Server, versioned entitlement snapshots, canonical module toggles/presets и backend gates для текущих `cloud-subscription`/`table-mode`/`kitchen-space`/`warehouse-mode`/`ticket-mode` surfaces реализованы сейчас; canonical contract находится в `docs/backend/LICENSE-ENTITLEMENTS.md`. Tenant-level roles/employees, employee restaurant memberships, `organization.manage`, tenant catalog identity и restaurant menu overrides реализованы сейчас. QR-enabled service items (POS-52): `qr_confirmation_enabled`, auto-derived `single_unit_per_line`, и `validity_mode` (`cash_session`/`business_date`/`absolute_date`) реализованы сейчас в Cloud catalog, EdgeCatalogItem delivery, POS Edge order validation (qty > 1 блокируется backend) и Cloud UI форме. Ticket issuance (POS-48) реализовано сейчас: при включенном `ticket-mode` после полной оплаты и закрытия final check для каждой QR-enabled line транзакционно создается одна `ticket_units` единица с immutable name/sale date/timezone/validity, уникальным UUIDv7 ticket number, порядковым номером внутри кассовой смены и QR payload `MHT1:<ticket_number>` без secrets; replay не создает второй билет, reprint (`POST /api/v1/tickets/{id}/reprint`) использует тот же ticket number и QR с COPY marker без новой единицы, а `TicketIssued` синхронизируется Edge -> Cloud как accepted audit (operational sales projection/API — POS-40). QR checker lookup/confirm/revoke — в post-deploy цикл. Целевой контракт описан в `docs/project-management/EXHIBITION-ALPHA-PILOT-REQUIREMENTS.md`.
...
- Receipt preview endpoint `GET/POST /api/v1/receipts/preview` реализовано сейчас для ReceiptLine Level 1: принимает `template_content`, `document_type`, `cpl` 32/48 и `print_context` JSON object, рендерит шаблон через `engine.Render` и возвращает `image/svg+xml`; POST вариант используется Cloud UI editor (POS-80); как tenant-level Cloud surface требует `cloud-subscription`.
- Cloud-owned ESC/POS принтеры (POS-82/POS-83/POS-84) реализовано сейчас: таблица `cloud_printers` в managed baseline, CRUD `/api/v1/printers` под permission `organization.manage` с soft-delete (is_active=FALSE, version++); Cloud -> Edge mastersync stream `printers` доставляет все активные принтеры ресторана с checkpoint `printers:{restaurant_id}:{MAX(updated_at).UnixMilli()}:{count}`; поля: `type` (tcp|usb), `address`, `port` (NULL для USB), `document_types` (JSON array), `codepage` (''/cp437/cp866), `paper_cut_type` (full|partial), `cpl` (32|42|48|56|80). Clean-stack аудит 2026-06-28 исправил Cloud baseline/exchange validation для stream `printers`; Cloud runtime version `0.1.15`. POS Edge атомарно заменяет строки `receipt_printers` по `restaurant_id`. **Обновлено POS-86:** print worker больше не матчит принтер напрямую по `restaurant_id + document_type` — маршрутизация идёт строго через `print_routes` (см. следующий пункт), старый способ убран без fallback. Cloud UI `printers` route (restaurant scope) реализован в `cloud-ui-g`: список принтеров + inline create/edit форма с Zod-валидацией response, i18n строками и safe error banner; деактивация через DELETE `/printers/{id}`.
- Полная модель ресторанной схемы печати (POS-85+POS-86) реализована сейчас: `sales_points`/`restaurant_sections`/`tables.section_id` — Cloud-owned master data (CRUD в cloud-backend, read-only ingest на Edge через mastersync), `print_routes`/`print_job_targets`/`printer_route_override_audit` — полностью Edge-local. Маршрутизация печати строго по фиксированной привязке `document_type→scope_type` (`check_nonfiscal→sales_point`, `precheck`/`ticket`→`section(hall_section)`, без fallback), продублированной DB-триггерами. Claim и retry — per-printer FIFO buffer на уровне `print_job_targets`, `print_jobs.status`/`attempts` агрегируются из targets. Кассовая смена обязана быть привязана к точке продаж (`cash_sessions.sales_point_id NOT NULL`); точка продаж никогда не создаётся автоматически, только вручную через Cloud CRUD или seed. Печать стала гейтом подтверждения оплаты: после оплаты enqueue `check_nonfiscal` (не `precheck` — фикс gap POS-72), `checks.print_confirmed_at` стампится worker'ом, есть retry подтверждения (пересборка targets из текущих routes) и `cancel-unconfirmed` (manager PIN, компенсирующая финансовая операция + void билетов + soft-cancel заказа). Старый Edge-local `EnsureSystemFloor`/`GetSystemTable`/`__counter__` (POS-53) полностью ретирован — заказ без явного стола резолвит `salesPoint.DefaultTableID`. POS runtime version `0.1.11`. Подробности — `docs/backend/EDGE-PRINT-ROUTING-SPEC.md`. **Live-прогон 2026-07-01 (Windows/Docker Desktop, отдельная сессия после доводки):** `docker compose down -v && up -d --build` с нуля, `scripts/seed-dev-system.py --run-minimal-flow --run-kitchen-process-smoke` прошёл полностью зелёным через реальный HTTP-контур (создаёт `sales_point`, `print_routes` для `check_nonfiscal`/`precheck`/`ticket`, оба print job дошли до `failed` — ожидаемо без реального принтера, worker подтверждённо не завис). Playwright (реальный Chrome/Edge через MCP) прогнан по `cloud-ui-g`/`pos-ui-g` dev-серверам поверх того же стека: форма создания стола в `TablesPanel.tsx` теперь корректно собирает `section_id` и `POST /master-data/floor/tables` возвращает 201 (регресс из прошлой сессии не воспроизвёлся — фикс подтверждён живьём); экран Printers (POS-83) create/edit прошли штатно, а деактивация (`DELETE /printers/{id}`) ловила новый баг — **найдено и исправлено в этой сессии**: `localCORS` в `cloud-backend/internal/cloudsync/api/router.go` не включал `DELETE` в `Access-Control-Allow-Methods`, браузерный preflight блокировал запрос; после фикса деактивация возвращает 200. Открытие смены через `POSCashSection.tsx` без `sales_point_id` подтверждено падает 400 с safe error contract (`VALIDATION_FAILED`/`errors.validation`/`correlation_id`, без raw Go/SQL деталей) — соответствует ожиданию, экран сознательно не обновлён (see POS-88). Обойдя регрессию прямым API-вызовом (`sales_point_id` явно), полный флоу заказ → добавление позиции с модификатором → precheck → оплата (наличные) пройден через реальный UI до конца, экран оплаты не падает и показывает "Оплата принята, заказ закрыт" несмотря на реальный сбой печати (нет физического принтера). Обновлено по операторской Windows-проверке 2026-07-01: Windows-сборка собирается и запускается, печать через устанавливаемую очередь печати подтверждена; для POS-86 это закрывает hardware/manual-validation на уровне альфа-функционала. Не реализовано сейчас: Edge settings UI для `print_routes`/targets (`POS-88`, переформулирована), cloud-ui-g экран для точек продаж/секций (`POS-102`), Cloud projection Edge overrides (`POS-87`), offline-флоу автономного Edge (`POS-101`), обновление `pos-ui-g` под обязательный `sales_point_id` при открытии смены и под print-confirmation gate на экране оплаты. Дальнейшее production-hardening физической адресации и обнаружения принтеров вынесено во внешний `windows-printers` adapter после первого пилота.
- Receipt template master-data и Edge print queue (POS-71/POS-74/POS-84) реализовано сейчас: Cloud-owned таблица `cloud_receipt_templates` и CRUD `/api/v1/receipt-templates` под permission `cloud.templates.manage` с soft-delete и version increment; Cloud -> Edge stream `receipt_templates` в `mastersync.Service` собирает эффективный набор (restaurant-specific шаблоны плюс tenant-level defaults), Edge атомарно заменяет строки `receipt_templates` и хранит только активные, с stream-specific version checkpoint. `shared/platform/receipt/engine.Render` раскрывает ReceiptLine Level 1 `{if}`/`{each}`/`{{.field}}`/`| money` в IR для `precheck` и `ticket`; default templates `default_precheck.rl` и `default_ticket.rl` переведены на English (Indonesia launch) и сидируются через `scripts/seed-dev-system.py`. POS Edge `print_jobs` хранит локальную очередь без payload, in-process worker выполняет pipeline `receipt_printers` routing -> `receipt_templates` -> POS-72 projection -> `engine.Render` -> IR -> `escpos.Render` -> TCP/USB raw send с retry 3 попытки. ESC/POS рендерер использует **CP437 по умолчанию** (`ESC t 0`, USA/Standard Europe); явный `"codepage": "cp866"` в Cloud printer config включает CP866 для кириллицы. Cloud UI редактор шаблонов (POS-80) реализован сейчас: route `/receipt-templates` в Cloud Manager с двухпанельным интерфейсом (список + Monaco-style textarea) и live SVG preview. Fiscal logic, PNG/e-receipt и Level 2 templates — вне текущего объема.
...
- В `cloud-ui-g` реализованы route-backed разделы dashboard, restaurants, Edge sync, catalog, menu, modifiers, pricing/taxes, staff/permissions, floor, publications, **receipt-templates** (редактор шаблонов печати с live SVG preview, POS-80) и **printers** (управление Cloud-owned ESC/POS принтерами ресторана, POS-83).
...
- `scripts/seed-dev-system.py` проверяет health Cloud/POS/License, создает полный Cloud-owned seed dataset, сидирует default `receipt_templates` (`precheck`, `ticket`) через HTTP CRUD `/api/v1/receipt-templates`, создает active recipe versions через manager draft -> submit -> approve flow, выполняет license pairing POS Edge и проверяет базовый POS read model/Cloud -> Edge sync status без прямых DB writes и без publish API.
- `scripts/seed-dev-system.py --run-minimal-flow` выполняет минимальный HTTP-only smoke: Cloud recipes/stop-list/receipt_templates publication, Edge sync, waiter order/precheck, KDS served, cashier payment/final check, enqueue `print_jobs` для предчека и выданных ticket units, прием `ItemServed`/`CheckClosed` в Cloud, появление строк Cloud `stock_ledger` по `ItemServed`, появление materialized `stock-balances`, отсутствие duplicate `CheckClosed` delta для того же `order_line_id`, экспорт событий в ClickHouse `raw_business_events`, экспорт складских движений в `olap_stock_moves` и bounded reads `stock-move-summary`/`sales-kitchen-summary` без raw payload.
...
- Лицензионная фильтрация реализована сейчас: Edge -> Cloud batch содержит только module-owned события включенных модулей по ownership из `docs/backend/LICENSE-ENTITLEMENTS.md`, а Cloud receiver повторно reject-ит выключенные module events item-level ACK; базовые cashier financial facts остаются синхронизируемым ядром подключенного Cloud-контура.
...
- Deploy-подготовка 30.06.2026 реализована сейчас как alpha/pre-Kubernetes path:
  - `docs/deployment/CLOUD-CLIENT-DOCKER-TRAEFIK.md` и `deploy/cloud-client/*`/`deploy/traefik/*` описывают 1-5 изолированных Cloud client stacks на одной Linux VM через Docker Compose и Traefik, с Cloud UI на `/`, Cloud API на `/api/v1`, immutable image tag и внешним License Server;
  - `docs/deployment/LICENSE-SERVER-LINUX.md`, `deploy/license-server/*` и `scripts/deploy-license-server.sh` описывают native License Server на Ubuntu 24.04 под systemd, firewall/IP allowlist, startup backup и GitHub Actions workflow template;
  - `docs/deployment/POS-EDGE-WINDOWS.md`, `installer/nsis/pos-edge.nsi`, `scripts/build-pos-edge-installer.ps1` и `scripts/package-pos-edge-windows.sh` описывают portable/NSIS Windows package для POS Edge, install-time config, сохранение `data/`, optional `gowebwallpaper` kiosk host и `amd64` primary target.
...
- До первого выставочного запуска: Cloud sales dashboard/API (`POS-40`/`POS-41`), Telegram worker (`POS-63`), финальный deployment/backup/go-no-go smoke (`POS-43`/`POS-44`/`POS-45`) и оставшиеся production hardening checkpoints. Ticket issuance, уникальный номер, validity snapshot, QR payload и физическая нефискальная ESC/POS печать precheck/ticket реализованы сейчас (`POS-48`/`POS-64` P1); hardware стенд `10.25.1.201:9100` подтвердил выход двух чеков, ticket после исправления эффективной ширины для крупного текста требует повторной визуальной проверки. Внешний licensing authority и gates существующих `cloud-subscription`/`table-mode`/`kitchen-space`/`warehouse-mode`/`ticket-mode` surfaces реализованы сейчас; tenant-level roles/employees, memberships, tenant catalog identity и restaurant menu overrides также реализованы сейчас. POS license flow после ручного аудита вынесен в `POS-95` и находится в `Review`.
