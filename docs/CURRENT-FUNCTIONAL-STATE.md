...
- Cloud-owned ESC/POS принтеры (POS-82/POS-83/POS-84) реализовано сейчас: таблица `cloud_printers` в managed baseline, CRUD `/api/v1/printers` под permission `organization.manage` с soft-delete (is_active=FALSE, version++); Cloud -> Edge mastersync stream `printers` доставляет все активные принтеры ресторана с checkpoint `printers:{restaurant_id}:{MAX(updated_at).UnixMilli()}:{count}`; поля: `type` (tcp|usb), `address`, `port` (NULL для USB), `document_types` (JSON array), `codepage` (''/cp437/cp866), `paper_cut_type` (full|partial), `cpl` (32|42|48|56|80). Clean-stack аудит 2026-06-28 исправил Cloud baseline/exchange validation для stream `printers`; Cloud runtime version `0.1.15`. POS Edge атомарно заменяет строки `receipt_printers` по `restaurant_id`, а print worker читает routing из SQLite по `restaurant_id + document_type`; один document type может печататься на нескольких активных принтерах, отсутствие принтера завершает job безопасным кодом `PRINT_ROUTING_NOT_CONFIGURED`. Cloud UI `printers` route (restaurant scope) реализован в `cloud-ui-g`: список принтеров + inline create/edit форма с Zod-валидацией response, i18n строками и safe error banner; деактивация через DELETE `/printers/{id}`.
- Запланировано далее для полноценной ресторанной схемы печати: `sales_points` с обязательными `name` и `analytics_tag`, кассовая смена в рамках точки продаж, обязательный принтер кассы для точки продаж, `restaurant_sections` режимов `hall_section`/`kitchen_workshop`, назначение сервисных принтеров на секции, принтер отчетов на ресторан, Edge settings для добавления принтеров/назначений/очереди, Edge-originated printer/route override с немедленным локальным применением, audit и push в Cloud. Для нескольких принтеров одного документа нужен `print_job_targets` с отдельным статусом/retry/error/printed_at на каждый принтер. Не реализовано сейчас: runtime сущности `sales_point`, `restaurant_section`, `print_routes`, `print_job_targets`, Edge settings UI для этих операций и Cloud projection Edge overrides.
...
- До первого выставочного запуска: Cloud sales dashboard/API (POS-40), Telegram worker и оставшиеся production hardening checkpoints. Ticket issuance, уникальный номер, validity snapshot, QR payload и физическая нефискальная ESC/POS печать precheck/ticket реализованы сейчас (POS-48/POS-64 P1); hardware стенд `10.25.1.201:9100` подтвердил выход двух чеков, ticket после исправления эффективной ширины для крупного текста требует повторной визуальной проверки. Внешний licensing authority и gates существующих table/kitchen/warehouse surfaces реализованы сейчас; tenant-level roles/employees, memberships, tenant catalog identity и restaurant menu overrides также реализованы сейчас.
