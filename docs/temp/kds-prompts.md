...
**Итерация 16: KDS Stop-List Edit UI + Sync Pending Indicator**
Выполнена в bounded UI/runtime объеме. POS Edge добавил safe read endpoint `GET /api/v1/kitchen/stop-list` под `pos.kitchen.stop_list.view`: ответ содержит локальный stop-list overlay и агрегированный outbox sync state (`pending`, `acknowledged`, `problem`, `cloud_authority`, `unknown`) без `payload_json` и raw error body. `pos-ui-g` kitchen mode добавил вкладку `Стоп-лист`, backend-backed форму stop/resume поверх `POST /api/v1/kitchen/stop-list-updates`, loading/error/empty/no-permission handling через существующие primitives и indicator pending/ack/problem из backend DTO. Sale blocking остается backend-authoritative; full inventory/costing, production-grade review workflow, backfill/operator UI и rich KDS analytics остаются вне текущего объема.

**Итерация 17: Scenario Recipe Version Editor/Review + Stop-List Regression Hardening**
Выполнена в bounded Cloud-owned объеме. Cloud добавил `cloud_recipe_versions`/`cloud_recipe_lines`, routes `GET /api/v1/master-data/recipes/versions`, `POST /api/v1/master-data/recipes/versions/drafts`, `POST /api/v1/master-data/recipes/versions/{id}/submit`; submit создает pending `RecipeChangeSuggested`, approve активирует новую версию через Cloud authority и публикует `recipes` package, POS Edge остается read-only consumer. Cloud UI добавил минимальный route-backed экран версий техкарт: текущие версии, draft form с одной компонентной строкой, save draft / submit, ошибки через backend contract, review остается в существующей очереди предложений. POS Edge regression усилен: Cloud-imported active stop-list block не снимается локальным inactive Edge overlay, sale blocking по активному recipe component остается backend-authoritative. Full inventory engine, recipe expansion consumption, modifier-linked consumption, costing/COGS/margin и production-grade analytics остаются вне текущего объема.

**Итерация 18: Cloud Inventory Balances + Costing Status Foundation**
Выполнена в bounded Cloud-owned объеме. Cloud добавил `GET /api/v1/inventory/stock-balances?restaurant_id=&warehouse_id=&catalog_item_id=&business_date_to=&costing_status=&limit=&offset=` как read model поверх PostgreSQL `stock_ledger`: отрицательные остатки допустимы, `costing_status` агрегируется в `final`/`estimated`/`needs_recalculation`/`mixed`/`unknown`, response не содержит raw payload, COGS или margin. Cloud UI inventory surface больше не является только readiness-only: показывает таблицу остатков и costing status с фильтрами, сохраняя readiness signals stop-list. `scripts/seed-dev-system.py --run-kitchen-process-smoke` дополнительно читает `stock-balances` после kitchen receipt/count/write-off/production и проверяет отсутствие raw payload. POS Edge остается генератором events/read-only consumer и не получает stock balances/costing authority; full inventory engine, retro recalculation DAG, recipe expansion consumption, modifier-linked consumption, COGS/margin, BI dashboards и production-grade backfill/operator UI остаются вне текущего объема.
