...
Выбор скидки из Cloud-authored policy требует `pos.pricing.discount.apply`; выбор надбавки требует `pos.pricing.surcharge.apply`; просмотр списка активных policies требует `pos.pricing.view`. Если policy содержит `requires_permission`, backend дополнительно проверяет это право в operator session.
