...
- hardware bump-bar/printer orchestration и rich KDS analytics не описываются как реализованные.
...
- В React/Vite `pos-ui-g` раздел `activity` получает already-loaded `closedOrders` из `POSContext`; `POSContext` вызывает `listClosedOrders(50)`, UI локально показывает страницы по 6 строк, поиск работает по загруженному ограниченному набору, а operator-facing archive/retention/export/compaction controls не реализованы.
...
- pure helpers KDS вынесены в `pos-ui-g/src/components/kitchen/kitchenHelpers.ts`; presentational слой очереди и proposal list вынесен в `KitchenOrdersTab` и `KitchenProposalList`; runtime component остается владельцем state/loaders/submit handlers и не меняет backend contracts или allowed transitions;
...
- `/pos/kitchen` не покрывает bump-bar/printer orchestration и rich KDS analytics.
...
- запланировано далее: bump-bar/printer orchestration and rich KDS analytics;
