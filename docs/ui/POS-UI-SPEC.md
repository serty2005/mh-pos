...
- closed orders UI использует bounded backend pagination: business-date filter, `limit=26` для page-size 25 + next probe, `offset` previous/next controls; клиентский поиск применяется только к текущей странице и не является full-history search;
