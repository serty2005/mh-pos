...
- full check cancellation и full check refund для closed orders через ledger endpoints с cashier reason и выбором inventory disposition;
...
- Cashier UI has pilot-minimum full check cancellation through `POST /api/v1/checks/{id}/cancellations` guarded by `pos.precheck.cancel` and an open current cash session that belongs to the original shift.
- Cashier UI has pilot-minimum full check refund through `POST /api/v1/checks/{id}/refunds` guarded by `pos.payment.refund`, captured payment presence and an open current cash session different from the original payment shift.
- Диалог full check cancellation/refund отправляет `command_id`, reason, `operation_kind: full` и выбранный `inventory_disposition`; backend владеет totals, remaining compensable amount и final operation items.
- Partial ledger scopes `order_line`, `modifier_line`, `service_charge`, `tip` и `payment` показаны только как запланированные disabled scope options; runtime selection в cashier UI не реализован.
- Cashier UI keeps the compatibility route `POST /api/v1/payments/{id}/refund` as an explicit payment-level fallback for closed orders with captured payments.
...
- partial check/line/quantity cancellation UI;
