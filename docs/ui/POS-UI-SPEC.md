...
- Разделы `Активность`, `Отчеты`, `Касса` разнесены минимально: активность открывает поддержанный closed orders/refund/reprint drawer, отчеты показывают безопасный placeholder для неподдержанных расширенных отчетов, касса содержит смены, cash session, cash drawer и sync.
