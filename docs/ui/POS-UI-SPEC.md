...
- В `pos-ui-g` реализовано сейчас: shell bottom navigation/icon controls, floor hall tabs/table tiles/active-order rail, order category/search/filter controls/menu tiles/order rail header, action-dialog course/modifier selectors, activity search/check rows/status badges, cash rail header/ledger badges и KDS catalog picker/status badges используют shared primitives. Отдельные financial dialogs и узкоспециализированные rows могут оставаться локальными, когда layout или тестовые IDs завязаны на сценарий.
...
- pure helpers KDS вынесены в `pos-ui-g/src/components/kitchen/kitchenHelpers.ts`; runtime component остается владельцем state/loaders/submit handlers и не меняет backend contracts или allowed transitions;
