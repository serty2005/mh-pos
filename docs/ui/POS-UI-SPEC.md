...
- detail закрытого заказа читает `GET /api/v1/checks/{id}/financial-operations` и показывает финансовые операции по выбранному final check: type, kind, amount, reason, employee/approver, business date, inventory disposition и created time;
...
- Backend ledger read capability for closed-order detail is implemented through `GET /api/v1/checks/{id}/financial-operations`.
