...
- primary cashier flow strip: `готовность смены -> стол -> заказ -> пречек -> оплата`;
- secondary operations (`cash drawer`, `closed orders`, `sync`) визуально отделены от основного пути продажи в utility-блоке/drawer;
...
- Primary flow и blocking notice вынесены в отдельные presentation components без изменения backend API calls.
