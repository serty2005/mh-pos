...
Реализовано сейчас: stop-list assignment endpoints возвращают только safe fields (`review_type`, `id`, `status`, assignment metadata) и не возвращают `payload_json`, raw Edge payload, PIN/token/secret/request dump. Assignment audit read реализовано сейчас на backend для `stop_list_update`, `catalog_suggestion` и `recipe_suggestion` как bounded routes с default `limit=50`, max `100`, `offset` non-negative и safe empty list для unknown review id; Cloud UI не обязан вызывать их в текущем scope, если backend API и tests закрывают задачу. Catalog/recipe assignment runtime и Cloud UI controls запланированы далее. Вне текущего объема и запрещено: raw payload exposure. Запланировано далее: escalation, dashboard и большой UX refactor.
