...
- Реализовано сейчас: Cloud-authored rows применяются на POS Edge через `POST /api/v1/sync/master-data/snapshots` или `POST /api/v1/sync/master-data/{stream}`. Поддержанные Edge ingest streams: `restaurants`, `devices`, `staff`, `floor`, `catalog`, `menu`.
...
- модульные версии берутся из единого `MH_POS_VERSION` (fallback `0.1.1`);
...
- если обнаружена ситуация `db version < module version`, отсутствующая version table или изменение данных через startup/data-load path, перед обновлением обязателен backup:
  - POS Edge (`SQLite`): SQLite online backup artifact `.db`;
  - Cloud (`PostgreSQL`): JSONL snapshot таблиц `public` схемы;
- Cloud -> Edge `full_snapshot` master-data import на POS Edge создает SQLite online backup до записи master rows и `cloud_master_sync_state`; `incremental` import backup не создает;
...
- реализованные сейчас POS Edge runtime columns из business-date/reprint итерации покрыты `002_runtime_schema_repair.sql`, потому что старая SQLite таблица не меняется через `CREATE TABLE IF NOT EXISTS`;
- реализованные сейчас Cloud projections (`cloud_projection_event_type_stats`, `cloud_projection_shift_finance`) имеют таблицы в PostgreSQL migrations и покрыты `003_runtime_schema_repair.sql`, потому что receiver runtime пишет в них при приеме Edge events;
