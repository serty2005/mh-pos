...
- `001_init.sql` остается единственным managed canonical SQL file для clean init и version-gated startup upgrade;
- служебные `schema_migrations` и `db_runtime_versions` создаются кодом startup framework;
- `schema_migrations` содержит имя active SQL file, checksum, status и `applied_at`;
...
- дополнительные SQL files не добавляются без отдельного архитектурного решения, tests, backup и профильной документации.
...
1. меняется canonical managed SQL file (`001_init.sql` для POS Edge SQLite, `001_sync_receiver.sql` для Cloud PostgreSQL);
...
- добавлять второй/третий SQL migration file only because “так привычнее”, без отдельного архитектурного решения, startup framework, tests, backup и schema verification;
...
- `schema_migrations` хранит active managed SQL file, checksum, status и время применения;
...
- если `schema_migrations` отсутствует или содержит старую запись без checksum, startup framework повторно применяет idempotent canonical SQL и записывает migration history только после успешного apply;
...
- после canonical SQL apply выполняется schema verification только реализованных сейчас runtime tables/columns/indexes;
...
- `schema_migrations` содержит один active managed SQL file и checksum;
- runtime tests не создают legacy payment-to-check coupling;
- precheck lifecycle constraints и outbox constraints не сломаны;
- schema verification contract совпадает с фактическим single-file `001` path;
- реализованные сейчас Cloud projections (`cloud_projection_event_type_stats`, `cloud_projection_shift_finance`) имеют таблицы в canonical PostgreSQL SQL file, потому что receiver runtime пишет в них при приеме Edge events;
