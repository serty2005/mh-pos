...
- Cloud baseline `0.1.12` переносит прежний `cloud_employees.restaurant_id` в memberships, удаляет restaurant ownership из roles/employees и снимает legacy `restaurant_id <> ''` constraints с foundation-таблиц tenant catalog. Совпадающие role names получают deterministic suffix из role ID. Startup выполняет обязательный PostgreSQL backup до checksum/version upgrade и проверяет membership table/columns/index до запуска HTTP/workers.
