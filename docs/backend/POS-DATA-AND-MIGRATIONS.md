...
- Backend предоставляет read-only apply readiness gate через `POST /api/v1/storage/archive/apply-readiness`; он агрегирует archive verification, runtime blockers, pending outbox, open operational boundaries и protected data flags, но не является destructive apply.
...
- Export-only archive может создать файл даже при destructive block. `destructive_apply_supported = false`, `blocked = true`; non-sent `edge_to_cloud` outbox rows по candidate aggregates добавляют `pending_edge_to_cloud_outbox`.
