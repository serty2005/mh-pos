...
- Archive verify принимает `archive_id` или `manifest_path`/`archive_path`, разрешает только пути внутри configured archive dir, проверяет manifest version, SHA-256 archive file, counts manifest vs JSONL, required identity fields, business-date range/cutoff consistency, `runtime_rows_deleted = false`, snapshot payload в `prechecks`/`checks` и summary-only payload policy для `local_event_log`/`pos_sync_outbox`. Verify возвращает `valid`, `errors`, counts/tables/business-date metadata и не читает/не мутирует active runtime tables.
