...
- `print_jobs` (POS-74, расширено POS-86) — локальная Edge очередь нефискальной печати в `pos-backend/migrations/sqlite/001_init.sql`: колонки `id`, `restaurant_id`, `document_type` (`precheck`, `check_nonfiscal`, `ticket`), `scope_id` (nullable, резолвится один раз при enqueue — `sales_point`/`section` id), `source_kind`, `source_id`, `status` (`pending`, `processing`, `succeeded`, `failed`, агрегируется из `print_job_targets`), `attempts` (агрегат из targets), `max_attempts`, `printer_class`, `last_error`, `next_attempt_at`, `locked_by`, `locked_at`, `printed_at`, `created_at`, `updated_at`; индексы `print_jobs_pending_due`, `print_jobs_restaurant_status_created`; unique `(document_type, source_id)` защищает от дублей при replay. Таблица не хранит print payload: worker читает immutable snapshot источника и template из `receipt_templates`.
- `sales_points`, `restaurant_sections`, `print_routes`, `printer_route_override_audit`, `print_job_targets` (POS-85, доведено до полной модели POS-86) — Edge read-replica точек продаж/секций (Cloud-owned, только read-only ingest через mastersync streams `sales_points`/`restaurant_sections`) и полностью Edge-local `print_routes`/`print_job_targets`/`printer_route_override_audit`. `sales_points.default_table_id NOT NULL`, `restaurant_sections.is_default`, `tables.section_id NOT NULL` (`tables.hall_id` стал декоративным nullable полем). DB-триггеры `print_routes_required_scope_*`/`print_routes_required_section_mode_*` и `tables_section_hall_mode_*`/`sales_points_default_table_*` защищают инварианты на уровне схемы. `cash_sessions.sales_point_id` стал `NOT NULL`. POS runtime version `0.1.11`. Полная модель — `docs/backend/EDGE-PRINT-ROUTING-SPEC.md`.
- `checks.print_confirmed_at`, `orders.cancelled_at`, `ticket_units.status` (`active|voided`), `manager_override_audit.action` (расширен значением `cancel_unconfirmed_order`) — POS-86 print-confirmation gate и cancel-unconfirmed compensating flow, см. `docs/backend/EDGE-PRINT-ROUTING-SPEC.md`.
- `sales_points`, `restaurant_sections`, `print_routes`, `printer_route_override_audit`, `print_job_targets` (POS-85) — реализовано сейчас как Edge SQLite baseline для ресторанной схемы печати. `sales_points` принадлежат ресторану, содержат обязательные `name` и `analytics_tag`, а `cash_sessions.sales_point_id` добавлен как nullable подготовительная связь. `restaurant_sections` имеют режим `hall_section` или `kitchen_workshop`. `print_routes` связывает document type и scope (`restaurant`, `sales_point`, `section`) с физическим `receipt_printers.id`. `printer_route_override_audit` хранит Edge override snapshots. `print_job_targets` хранит отдельный статус/retry/error/printed_at для каждого физического принтера, выбранного схемой печати. Runtime worker начнет создавать и исполнять targets в POS-86.
...
- Реализовано сейчас в Cloud Inventory Worker: `RefundRecorded`/`CancellationRecorded` с operation-level `inventory_disposition` нормализуются из immutable operation/check/precheck snapshots и создают append-only stock movements: `return_to_stock` -> `RETURN/IN`, `write_off_waste` -> `WASTE/OUT`, `no_stock_effect` -> без queue/ledger effect, `manual_review` -> failure queue item для ручного разбора. POS Edge не мутирует локальные stock tables и не меняет finalized payment/precheck/check.
...
- Backend предоставляет non-destructive archive verification/read/lookup foundation через `POST /api/v1/storage/archive/verify`, `POST /api/v1/storage/archive/read-plan` и `POST /api/v1/storage/archive/lookup`.
- Backend предоставляет destructive apply через `POST /api/v1/storage/archive/apply-plan`; apply выполняется только после verified JSONL archive, clean scoped outbox и отсутствия open operational boundaries для cutoff.
- Backend предоставляет read-only apply readiness gate через `POST /api/v1/storage/archive/apply-readiness`; он агрегирует archive verification, runtime blockers, pending outbox, open operational boundaries и protected data flags без удаления runtime rows.
...
- Dry-run считает candidate rows только для closed orders с `checks.business_date_local < cutoff_business_date_local`; cutoff должен использовать формат `YYYY-MM-DD` и не может быть будущей business date.
...
- `archive.jsonl` является typed JSONL: каждая строка содержит `table` и `row`. Включаются closed orders, order lines/modifiers/discounts/surcharges, связанные `kitchen_tickets`/`kitchen_ticket_events`, prechecks and breakdown tables, payments/payment attempts, checks, financial operations/items.
...
- Export-only archive может создать файл даже при destructive block. `destructive_apply_supported = true`, `blocked` и `block_reasons` отражают текущий runtime safety gate; non-sent `edge_to_cloud` outbox rows по candidate aggregates добавляют `pending_edge_to_cloud_outbox`.
- Archive verify принимает `archive_id` или `manifest_path`/`archive_path`, разрешает только пути внутри configured archive dir, проверяет manifest version, SHA-256 archive file, counts manifest vs JSONL, required identity fields, business-date range/exclusive cutoff consistency, `runtime_rows_deleted = false`, snapshot payload в `prechecks`/`checks` и summary-only payload policy для `local_event_log`/`pos_sync_outbox`. Verify возвращает `valid`, `errors`, counts/tables/business-date metadata и не читает/не мутирует active runtime tables.
- Archive read-plan выполняет ту же verification и возвращает bounded preview archived closed orders: default `limit=50`, max `limit=100`, `offset`, filters `business_date_local`, `order_id`, `check_id`, flags `runtime_restored = false` and `runtime_rows_deleted = false`. Read-plan не импортирует строки в active SQLite, не пишет archive tables/runtime tables, `local_event_log` или `pos_sync_outbox` и не раскрывает sync/event payload JSON.
- Archive lookup принимает `archive_id` или `manifest_path`/`archive_path` и ровно один ключ `check_id` или `order_id`, перед lookup выполняет ту же verification и streaming-способом возвращает immutable check/precheck snapshot preview и related counts. Lookup не пишет archive tables, runtime tables, `local_event_log` или `pos_sync_outbox` и не раскрывает summary payloads outbox/local events.
...
- Archive apply-plan: при verified archive + runtime safety (нет pending scoped outbox, нет open boundaries для cutoff) выполняет destructive apply + VACUUM (реализовано сейчас), возвращает `result_mode = destructive_apply`, `runtime_rows_deleted = true`; иначе `apply_blocked` с block_reasons.
- Archive apply-readiness использует ту же read-only проверку, но возвращает policy-oriented контракт: `result_mode = apply_readiness_only`, `ready_for_destructive_apply`, `archive_verified`, `manifest_verified`, `snapshot_payload_verified`, `runtime_scope_verified`, `blocking_outbox_count`, `pending_edge_to_cloud_outbox`, `open_operational_boundaries`, `protected_data` и `block_reasons`.
- Apply-plan не отключает FK. Для append-only financial ledger triggers destructive transaction временно снимает delete triggers внутри той же SQLite transaction boundary и восстанавливает их до commit; rollback восстанавливает исходное состояние. Apply-plan не пишет archive tables и не выполняет restore.
- После успешного commit apply-plan запускает `VACUUM` через отдельное SQLite connection setup в WAL runtime mode. Apply-readiness не удаляет runtime rows и не запускает `VACUUM`/compaction.
...
- background auto-archive и automatic restore из archive в active SQLite.
