...
- Для linked modifier consumption выбран консервативный runtime contract: quantity linked item = sold line delta quantity * selected modifier quantity, а linked catalog item списывается напрямую и не разворачивается в recipe.
...
- `GET /api/v1/inventory/stock-balances` реализовано сейчас как bounded Cloud-owned materialized read model поверх `inventory_stock_balances`: фильтры `restaurant_id`, `warehouse_id`, `catalog_item_id`, `business_date_to` как UTC-дата `last_movement_at`, `costing_status`, bounded `limit/offset`, stable sort, отрицательные остатки допустимы, response не содержит raw Edge payload, COGS или margin.
...
Минимальная структура `inventory_stock_balances`, реализовано сейчас:

| Поле | Тип | Комментарий |
|---|---|---|
| `restaurant_id` | TEXT | ресторан |
| `warehouse_id` | TEXT | склад; пустая строка используется как normalized no-warehouse identity |
| `catalog_item_id` | TEXT | товар/ингредиент |
| `unit_code` | TEXT | machine code единицы |
| `quantity_on_hand` | NUMERIC(14,3) | текущий materialized остаток; отрицательные значения разрешены |
| `last_movement_at` | TIMESTAMPTZ | timestamp последней примененной ledger row |
| `last_ledger_entry_id` | TEXT | ссылка на последнюю примененную `stock_ledger.id` |
| `costing_status` | TEXT | deterministic aggregate: `final`, `estimated`, `needs_recalculation`, `recalculated`, `failed` |
| `needs_recalculation` | BOOLEAN | true для aggregate status `needs_recalculation` или `estimated` |
| `created_at` | TIMESTAMPTZ | время создания row |
| `updated_at` | TIMESTAMPTZ | время последнего обновления row |

Identity и индексы:

- уникальная identity: `restaurant_id`, `warehouse_id`, `catalog_item_id`, `unit_code`;
- фильтры: `restaurant_id`, `warehouse_id`, `catalog_item_id`, UTC-дата `last_movement_at` через bounded `business_date_to`, `costing_status`;
- `last_ledger_entry_id` указывает на уже записанную immutable ledger row.
...
erDiagram
  ...
  STOCK_LEDGER ||--o{ INVENTORY_STOCK_BALANCES : "updates materialized"
...
Реализовано сейчас: Cloud PostgreSQL baseline содержит nullable `linked_catalog_item_id` в `cloud_modifier_options`; Cloud master data валидирует, что ссылка указывает на существующий catalog item того же ресторана. Publication DTO передает поле в `modifier_options[]`, а POS Edge хранит его как read-only reference без права создавать stock documents, stock moves, stock balances или costing state.

Реализовано сейчас: Cloud Inventory Worker при `CheckClosed` генерирует отдельную `SALE/OUT` строку stock ledger для linked catalog item. Quantity = sold line delta quantity * selected modifier quantity. Если `linked_catalog_item_id` отсутствует, modifier влияет только на цену и immutable snapshots. Linked modifier item списывается напрямую; recipe expansion применяется к основной позиции продажи и не применяется к linked modifier item в этой итерации.
...
`RefundRecorded` и `CancellationRecorded` в текущем Cloud Worker используют operation-level `inventory_disposition`. Cloud receiver ставит событие в `inventory_event_queue`, только если disposition не равен `no_stock_effect`. Worker нормализует текущие POS ledger `items[]` из immutable operation/check/precheck snapshots; already-normalized inventory fields также принимаются как безопасный input для Cloud-side тестов и будущих отправителей. Mixed-disposition сценарий должен быть разложен до однородных операций до попадания в worker.
...
2. Полная allocation-модель beyond текущего operation-level refund/cancellation disposition, semi-finished auto-production split и retro costing.
...
Себестоимость пишется в `stock_ledger.unit_cost_minor` и `total_cost_minor` на момент обработки события. Отрицательные остатки допустимы. Реализовано сейчас: запись `stock_ledger` и обновление `inventory_stock_balances` выполняются в одной Cloud PostgreSQL транзакции; replay того же source event не меняет balance повторно из-за уникальности `stock_documents(source_event_id, source_event_type)`.

Balance-level costing visibility агрегирует существующие `stock_ledger.costing_status` для identity `(restaurant_id, warehouse_id, catalog_item_id, unit_code)` по консервативному правилу:

1. Если есть `failed` -> `failed`.
2. Иначе если есть `needs_recalculation` -> `needs_recalculation`.
3. Иначе если есть `estimated` -> `estimated`.
4. Иначе если последний status по `occurred_at`/ledger id равен `recalculated` -> `recalculated`.
5. Иначе `final`.

`inventory_stock_balances.needs_recalculation` выставляется в `true`, если aggregate status равен `needs_recalculation` или `estimated`.
...
Реализовано сейчас: whole-check операции нормализуются Worker-ом из immutable check/precheck snapshot lines, а partial `order_line` использует quantity из operation item и catalog/order line из snapshot. `service_charge`, `tip`, `payment` и `modifier_line` без authoritative linked catalog item не создают stock movement; missing `catalog_item_id` дает safe no-movement outcome без panic. Если оператор выбрал разные решения по позициям, текущий backend slice требует разложить это в однородные операции до worker.
...
- Cloud Inventory Worker покрыт bounded recipe expansion для основной позиции продажи и modifier-linked consumption для `CheckClosed` payload modifiers; `CheckClosed` после обработанного `ItemServed` не создает повторные строки по той же order line delta.
- Cloud Inventory Worker покрыт тестами на `ItemServed` + `CheckClosed` dedup/delta и superseded `ItemServed`; POS KDS ticket lifecycle генерирует `ItemServed`, ClickHouse `raw_business_events` работает как event archive, bounded `stock-balances` покрыт API/repository tests на чтение materialized state, filters/bounds/stable sort/negative balance/deterministic costing/no raw payload, а полный costing engine остается вне текущего runtime.
...
- использовать `linked_catalog_item_id` как Edge-side stock authority; поле на POS Edge остается read-only reference.
