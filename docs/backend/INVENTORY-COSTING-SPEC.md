...
- ClickHouse `olap_stock_moves`, read-only export status и первый stock movement summary реализованы сейчас; mutating retry/backfill controls, COGS/margin, sales и kitchen timing API остаются `запланировано далее`;
...
- добавить production-grade mutating backfill/retry operator controls для ClickHouse projections;
