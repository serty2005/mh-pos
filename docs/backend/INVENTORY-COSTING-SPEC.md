...
- ClickHouse `olap_stock_moves`, read-only export status, минимальный support-only export retry control, первый stock movement summary и первый bounded sales/kitchen summary реализованы сейчас; Cloud UI читает только read-only export status/stock movement slices и не вызывает support-only retry. Production-grade backfill jobs/operator UI, COGS/margin, richer sales aggregates и production-grade kitchen timing API остаются `запланировано далее`;
...
- добавить production-grade backfill/retry jobs и mutating operator UI для ClickHouse projections; текущий Cloud UI ограничен read-only bounded slices;
