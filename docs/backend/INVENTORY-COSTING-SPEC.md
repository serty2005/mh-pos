...
- полноценный Edge-origin stop-list edit UI/review workflow поверх уже реализованного `StopListUpdated` projection/conflict policy;
...
- ClickHouse `olap_stock_moves`, read-only export status, минимальный support-only export retry control и первый stock movement summary реализованы сейчас; production-grade backfill jobs/operator UI, COGS/margin, sales и kitchen timing API остаются `запланировано далее`;
...
Статус: реализовано сейчас для POS Edge local sale blocking, Cloud -> Edge streams `recipes`/`inventory_reference`, async `StopListUpdated` projection и минимального `stop_list_conflict_policy`. Полноценный Edge manager/KDS edit UI и manager review workflow остаются запланированы далее.
...
Изменение stop-list приходит из Cloud manager UI через Cloud-owned master-data rows или, если Edge-side input будет включен, как `StopListUpdated`. Порядок применения Cloud package и Edge overlay задается `stop_list_conflict_policy`: `cloud_wins`, `edge_overlay_until_next_publication`, `edge_overlay_requires_manager_review`. Default: `edge_overlay_requires_manager_review`, чтобы Edge-origin изменение не перетирало Cloud-owned stop-list без отдельного manager review.

Реализовано сейчас: Cloud receiver валидирует `StopListUpdated`, кладет событие в durable `inventory_event_queue`, а Cloud Inventory Worker пишет `cloud_projection_stop_list_updates` без raw payload. Policy `edge_overlay_until_next_publication` обновляет bounded `stop_lists` overlay; `cloud_wins` и default `edge_overlay_requires_manager_review` не мутируют Cloud-owned stop-list row и фиксируются как безопасная projection. `GET /api/v1/sync/readiness/stop-list` возвращает publication/package metadata, последний accepted `StopListUpdated` ACK metadata и агрегаты sync problem events без raw payload.
...
- реализовать sales/kitchen/costing bounded Cloud OLAP API поверх ClickHouse projections;
