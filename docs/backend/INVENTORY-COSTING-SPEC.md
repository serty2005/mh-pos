...
PostgreSQL хранит транзакционный журнал и состояния пересчета. Реализовано сейчас: ClickHouse хранит immutable `raw_business_events` бессрочно, получает первый bounded slice `olap_stock_moves` из PostgreSQL `stock_ledger`, отдает read-only export status, первый bounded stock movement summary, sales/kitchen summary и kitchen timing summary. Cloud backend также отдает bounded `stock-balances` read model, рассчитанный из PostgreSQL `stock_ledger`, с aggregate costing status без raw payload. Cloud UI реализовано сейчас показывает bounded read-only `stock-balances`, OLAP export status, stock moves, stock move summary, backfill job status и kitchen timing summary; mutating retry/backfill controls, COGS, маржинальность, materialized balance engine и rich BI остаются `запланировано далее`, пока нет полного costing/recalculation engine, production auth/RBAC perimeter и нужных projection данных.
...
- ClickHouse `olap_stock_moves`, read-only export status, минимальный support-only export retry control, async backfill jobs foundation, первый stock movement summary, первый bounded sales/kitchen summary и bounded kitchen timing summary реализованы сейчас; Cloud UI читает read-only export status/stock movement slices/backfill job status/timing summary и не вызывает support-only mutating retry/backfill controls. COGS/margin и richer sales aggregates остаются `запланировано далее`;
...
- добавить production auth/RBAC perimeter и mutating operator UI для ClickHouse projections; текущий Cloud UI ограничен read-only bounded slices и backfill job status;
