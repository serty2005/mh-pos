...
- Реализовано сейчас: `POST /api/v1/print/jobs/{id}/retry` требует `pos.print.retry`, принимает пустой JSON body и переводит `failed` или `pending` job в `pending`, пересобирая `print_job_targets` из ТЕКУЩИХ активных `print_routes` (job-level retry, не просто сброс attempts).
- Реализовано сейчас: `POST /api/v1/print/jobs/{id}/targets/{target_id}/retry` требует `pos.print.retry` и сбрасывает один target без пересборки routing.
- Manual retry не повторяет payment, не создает ticket, не меняет order/check/payment/financial state; он мутирует только локальные строки `print_jobs`/`print_job_targets`.
- Реализовано сейчас (POS-86, заменяет описанную выше document_type-based маршрутизацию через `receipt_printers`): print worker маршрутизирует строго по `print_routes` (Edge-local CRUD) с фиксированной привязкой `document_type → scope_type` (`check_nonfiscal→sales_point`, `precheck`/`ticket`→`section(hall_section)`, без fallback на старую логику). Claim — на уровне `print_job_targets` (per-printer FIFO buffer), `print_jobs.status`/`attempts` агрегируются из targets. Если route не настроен, job/target получает безопасный `last_error = PRINT_ROUTING_NOT_CONFIGURED`. Полный контракт, схема владения данными (`sales_points`/`restaurant_sections`/`tables.section_id` — Cloud-owned; `print_routes` — Edge-local), print-confirmation gate (`checks.print_confirmed_at`, `GET/POST .../print-confirmation[/retry]`, `POST /orders/{id}/cancel-unconfirmed`) и обязательность `sales_point_id` у кассовой сессии — в `docs/backend/EDGE-PRINT-ROUTING-SPEC.md`.
- `PrintJob` response содержит `id`, `restaurant_id`, `document_type`, `scope_id`, `source_kind`, `source_id`, `status`, `attempts`, `max_attempts`, `printer_class`, `last_error`, `next_attempt_at`, `locked_by`, `locked_at`, `printed_at`, `created_at`, `updated_at`, `targets` (массив `PrintJobTarget`).
...
- При ручном Cloud assignment Edge сохраняет Cloud URL, который оператор указал при регистрации/polling; пустой `cloud_url` в assignment response не перезаписывает локальный адрес, а snapshot скачивается с уже настроенного Cloud endpoint.
