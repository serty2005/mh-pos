...
- destructive retention apply/delete, archive restore в active SQLite, `VACUUM`, compaction;
...
- Реализовано сейчас: `POST /api/v1/storage/archive/read-plan` принимает `manifest_path` и optional `archive_path`; endpoint проверяет, что пути находятся внутри configured archive dir, читает manifest и JSONL streaming-способом, проверяет `version = pos_storage_archive_export_v1`, SHA-256, counts manifest vs JSONL и snapshot payload в `prechecks`/`checks`, возвращает `result_mode = read_plan_only`, summary counts/tables/business-date range и verification flags без business payload.
- Read-plan block reasons включают `archive_manifest_missing`, `archive_manifest_invalid`, `archive_manifest_version_mismatch`, `archive_path_outside_archive_dir`, `archive_missing`, `archive_unreadable`, `archive_sha_mismatch`, `archive_manifest_counts_mismatch`, `archive_snapshot_payload_missing`.
- Реализовано сейчас: `POST /api/v1/storage/archive/lookup` принимает `manifest_path`, `archive_path` и ровно один lookup key: `check_id` или `order_id`; перед lookup выполняет ту же archive verification, затем streaming-способом ищет archived check/order без загрузки всего archive payload в память.
...
- Реализовано сейчас: `POST /api/v1/storage/archive/apply-plan` принимает `cutoff_business_date_local`, `archive_path`, `manifest_path` и optional `mode = plan_only`; endpoint read-only, не выполняет `DELETE`, `UPDATE`, `VACUUM`, `wal_checkpoint` или restore в active SQLite.
...
- Export-only archive, read-plan, lookup и apply-plan не выполняют `DELETE`, `UPDATE`, destructive apply, restore в active SQLite, `VACUUM`, `wal_checkpoint`, startup/background auto-archive или compaction. Non-sent `edge_to_cloud` outbox rows по candidate aggregates возвращаются как block reason для будущей destructive policy, но сам export file может быть создан.
- Не реализовано сейчас: физическое удаление, archive tables, restore в active SQLite, destructive apply и VACUUM/compaction как часть HTTP lifecycle flow.
...
- Advanced Kitchen API:
  - `GET /api/v1/kitchen/tickets`;
  - `POST /api/v1/kitchen/ticket-items/{id}/accept`;
  - `POST /api/v1/kitchen/ticket-items/{id}/start`;
  - `POST /api/v1/kitchen/ticket-items/{id}/hold`;
  - `POST /api/v1/kitchen/ticket-items/{id}/ready`;
  - `POST /api/v1/kitchen/ticket-items/{id}/served`;
  - `POST /api/v1/kitchen/ticket-items/{id}/recall`;
  - `POST /api/v1/kitchen/ticket-items/{id}/cancel`;
