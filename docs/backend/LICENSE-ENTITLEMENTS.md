...
Локальный seed через HTTP создает active snapshots для `cloud-local` и `edge-local`. Локальный Docker POS Edge дополнительно имеет fallback на `cloud-local`, чтобы ручная настройка одного общего local snapshot не блокировала `/api/v1/license/entitlements` и `/api/v1/halls` после Cloud assignment.
