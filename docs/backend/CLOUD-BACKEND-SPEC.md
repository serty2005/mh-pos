...
Реализовано сейчас: assignment поддержан только для Edge-origin stop-list review item из `StopListUpdated`. `assign` принимает UUIDv7 `command_id`, `assigned_to_employee_id`, `assigned_by_employee_id` и safe `reason`; `unassign` принимает UUIDv7 `command_id`, `unassigned_by_employee_id` и safe `reason`. Replay того же `command_id` идемпотентен и не добавляет повторную audit row. Terminal statuses `approved` и `rejected` нельзя назначать или снимать с назначения. Responses возвращают только safe assignment metadata и не содержат raw payload. Stop-list assignment audit read реализовано сейчас через bounded `GET /api/v1/manager/stop-list-updates/{id}/audit?limit=&offset=`: default `limit=50`, max `100`, `offset` non-negative, stable sort `occurred_at DESC, event_id DESC`; unknown review id возвращает safe empty list. Response содержит только `event_id`, `review_id`, `review_type`, `action`, `actor_employee_id`, `target_employee_id`, `occurred_at`, `reason` и safe `command_id`, без `payload_json`, raw payload, sync envelope, request dump, token/PIN/SQL details. Assignment audit для `catalog_suggestion` и `recipe_suggestion` запланирован далее. Escalation/dashboard запланированы далее. Raw payload exposure вне текущего объема и запрещено.
...
- Реализовано сейчас: backend assignment routes и bounded audit read доступны только для `stop_list_update` через `POST /api/v1/manager/stop-list-updates/{id}/assign|unassign` и `GET /api/v1/manager/stop-list-updates/{id}/audit?limit=&offset=`; assignment/audit для `catalog_suggestion` и `recipe_suggestion`, escalation и dashboard запланированы далее.
