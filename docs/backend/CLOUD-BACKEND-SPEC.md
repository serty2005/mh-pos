...
- Реализовано сейчас: `cloud_catalog_items` задает tenant catalog identity; `restaurant_id` у catalog item/folder/tag/item-tag link может быть пустым. `cloud_menu_items` является restaurant-scoped override layer и хранит stable `catalog_item_id`, `category_id` как menu folder/ticket category identity, `tag_id`, `tax_profile_id`, `availability_json`, lifecycle status и `runtime_status`.
