...
- POS-81 smoke в `scripts/seed-dev-system.py --run-minimal-flow` подтверждает, что после оплаты
  POS Edge видит доставленные шаблоны через sync status и ставит `print_jobs` для предчека и
  выданных ticket units. Физическая отправка на принтер остается hardware acceptance POS-64.
...
### Целевая модель `print_job_targets`

Запланировано далее: `print_jobs` остается родительской задачей печати документа, а
каждая фактическая отправка на физический принтер хранится отдельной строкой
`print_job_targets`.
...
Retry выполняется на уровне target. Родительский `print_jobs.status` становится
`succeeded`, когда все обязательные targets успешны, и `failed`, когда хотя бы один
обязательный target исчерпал retry policy. Ручной retry должен уметь перезапустить
один target без повторного payment, ticket issuance или изменения финансового state.
...
Windows USB/device path (`type=usb`, `address="\\\\.\\USB001"`) через существующие
`escpos.Open`/`escpos.WriteRaw` primitives.

## Целевая схема печати

Запланировано далее: физический принтер и назначение печати разделяются.
`receipt_printers` описывает физическое устройство, а схема печати связывает
документ с областью применения и одним или несколькими принтерами.
...
`sales_point` является restaurant-scoped справочником с обязательными `name` и
`analytics_tag`. Кассовая смена открывается в рамках точки продаж; device identity
остается технической привязкой и не заменяет точку продаж.
...
Edge settings должны позволять добавлять физические принтеры, назначать их на
точку продаж или секцию, просматривать очередь и повторять отдельный target.
Edge-side изменение схемы печати применяется сразу локально, записывается в audit
и отправляется в Cloud как Edge-originated override. Это не proposal flow.
