...
POS Edge реализовано сейчас пишет `StockWriteOffCaptured` как local event/outbox envelope. Cloud Go sync contract и Inventory Worker для этого event остаются запланировано далее.
...
Целевой Cloud worker создает `stock_documents.document_type = WASTE`, `stock_ledger.movement_type = OUT`; текущий Cloud runtime еще не принимает `StockWriteOffCaptured`.
