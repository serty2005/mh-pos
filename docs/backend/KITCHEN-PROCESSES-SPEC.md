...
- POS Edge routes для приемки поставки, ревизии, списания, приготовления заготовки, просмотра техкарты и отправки proposal events.
...
- POS Edge backend: kitchen application service, SQLite tables для локальных draft/proposal/receipt facts, routes, RBAC, outbox events.
...
- Каждая строка содержит `catalog_item_id` или `catalog_suggestion_id`.
- Если товара нет, UI сначала создает `CatalogItemChangeSuggested`, затем receipt line ссылается на `catalog_suggestion_id`.
...
Добавить новый event type `StockWriteOffCaptured`.
...
Cloud создает `stock_documents.document_type = WASTE`, `stock_ledger.movement_type = OUT`.
...
Добавить или расширить таблицы:
...
- `warehouse_reference` - Cloud -> Edge read model складов, минимум `id`, `restaurant_id`, `name`, `kind`, `active`, sync metadata.
...
Для строгой последовательности складских изменений добавить warehouse-aware processing:
...
Для кухонных событий складской worker полного контура использует ClickHouse stream как источник анализа последовательности. Перед созданием складского документа worker строит effective state по `restaurant_id`, `warehouse_id`, `order_line_id`, `ticket_id` и `serve_sequence`: если `ItemServed` был отозван переходом `served -> recall`, он не считается последней эффективной подачей. Если после recall пришел новый `ItemServed`, stock effect создается или пересчитывается только для последнего эффективного события.
...
Добавить stable error codes/message keys:
