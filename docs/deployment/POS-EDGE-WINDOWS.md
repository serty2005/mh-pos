...
NSIS installer создает Start Menu shortcut `MyHoreca\POS Edge`, который запускает `start-pos-edge.cmd` с рабочим каталогом установки и `POS_CONFIG_PATH=config\pos-edge.json`. Если installer собран с `-WebWallpaperExe`, дополнительно создается shortcut `MyHoreca\POS Edge Display`, который запускает `webwallpaper\gowebwallpaper.exe` с install-time config `webwallpaper\config.pos-edge.json`.
...
- shortcut `MyHoreca\POS Edge Display` передает этот config path в `gowebwallpaper.exe`, поэтому первый запуск не показывает URL prompt;
...
5. Запустить kiosk host через Start Menu shortcut `MyHoreca\POS Edge Display`.
...
Команда выше является ручным эквивалентом shortcut. При пустом `Monitors` в install-time config `gowebwallpaper` выбирает primary screen и сохраняет найденные мониторы в config без URL prompt. Другой экран можно выбрать позже через tray menu.
