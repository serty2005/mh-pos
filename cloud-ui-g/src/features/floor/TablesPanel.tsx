...
export default function TablesPanel({ halls, tables, loading, error, onCreate, onUpdate, onArchive }: Props) {
  const { t } = useI18n();
  const activeHalls = halls.filter((hall) => hall.status !== 'archived');
  const firstHallId = activeHalls[0]?.id ?? '';
  const [createValues, setCreateValues] = useState<TableFormValues>({ ...defaultTableValues, hall_id: firstHallId });
  const [editing, setEditing] = useState<RestaurantTable | null>(null);
  const [editValues, setEditValues] = useState<TableFormValues>(defaultTableValues);

  useEffect(() => {
    setCreateValues((prev) => ({ ...prev, hall_id: prev.hall_id || firstHallId }));
  }, [firstHallId]);
  ...
  const hallLabel = (hallId: string) => halls.find((hall) => hall.id === hallId)?.name ?? hallId;
    ...
    <form className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-4" onSubmit={(event) => { event.preventDefault(); void onSubmit(); }}>
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
        <div>
          ...
          <select value={values.hall_id} onChange={(event) => setValues({ ...values, hall_id: event.target.value })} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm" disabled={loading}>
      ...
      <button type="submit" disabled={loading || !values.hall_id || !values.name.trim()} className="rounded-lg bg-slate-900 px-3 py-2 text-sm font-medium text-white disabled:opacity-50">{actionLabel}</button>
    </form>
  );
  ...
  return (
    <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6">
      ...
      {renderForm(createValues, setCreateValues, async () => {
        ...
        setCreateValues({ ...defaultTableValues, hall_id: firstHallId });
        ...
        <article key={table.id} className="rounded-xl border border-slate-200 p-4">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div>
              ...
              <p className="text-xs text-slate-600">{hallLabel(table.hall_id)} · {table.seats} · {t(`catalog.statuses.${table.status}`)}</p>
            </div>
