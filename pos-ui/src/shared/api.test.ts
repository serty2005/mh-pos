...
describe('api request helpers', () => {
  ...
  it('sends selected modifiers when updating an order line', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      text: async () => JSON.stringify({
        id: 'line-1',
    ...
    await updateOrderLineModifiers('order-1', 'line-1', [{ modifier_group_id: 'group-1', modifier_option_id: 'option-1', quantity: 1 }]);

    expect(fetchMock.mock.calls[0]?.[0]).toContain('/orders/order-1/lines/line-1/modifiers');
    const init = fetchMock.mock.calls[0]?.[1] as RequestInit;
