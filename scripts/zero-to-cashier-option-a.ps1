param(
  ...
  [string]$ClientDeviceId = "zero-to-cashier-a"
)
...
$catalog = Invoke-JsonPost "$CloudApiBase/catalog/items" @{ restaurant_id = $restaurant.id; type = "dish"; name = "Zero Tea"; sku = "ZERO-A-TEA-$suffix"; base_unit = "portion" }
$menu = Invoke-JsonPost "$CloudApiBase/menu/items" @{ restaurant_id = $restaurant.id; catalog_item_id = $catalog.id; name = "Zero Tea"; price = 15000; currency = "RUB"; availability_json = "{}" }

$status = Invoke-RestMethod "$PosApiBase/system/provisioning-status"
Invoke-JsonPost "$CloudApiBase/restaurants/$($restaurant.id)/devices/$($status.node_device_id)/assign" @{} | Out-Null
...
if (-not $status.paired) { throw "Edge did not become paired." }

$login = Invoke-JsonPost "$PosApiBase/auth/pin-login" @{ node_device_id = $status.node_device_id; client_device_id = $ClientDeviceId; pin = $EmployeePin }
$headers = @{ "X-Node-Device-ID" = $status.node_device_id; "X-Client-Device-ID" = $ClientDeviceId; "X-Session-ID" = $login.session.id; "X-Actor-Employee-ID" = $login.actor.employee_id }
...
[pscustomobject]@{ restaurant_id = $restaurant.id; node_device_id = $status.node_device_id; employee_id = $employee.id; menu_item_id = $menu.id; table_id = $table.id }
