...
func (r *Repository) CreateCashSession(ctx context.Context, v *domain.CashSession) error {
	_, err := r.execer(ctx).ExecContext(ctx, `INSERT INTO cash_sessions(id,edge_cash_session_id,restaurant_id,device_id,shift_id,opened_by_employee_id,closed_by_employee_id,status,opening_cash_amount,closing_cash_amount,opened_at,closed_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
		v.ID, v.EdgeCashSessionID, v.RestaurantID, v.DeviceID, v.ShiftID, v.OpenedByEmployeeID, nullableString(v.ClosedByEmployeeID), string(v.Status), v.OpeningCashAmount, nullableInt64(v.ClosingCashAmount), dbTime(v.OpenedAt), nil, dbTime(v.CreatedAt), dbTime(v.UpdatedAt))
...
func (r *Repository) GetCashSession(ctx context.Context, id string) (*domain.CashSession, error) {
	return r.scanCashSession(r.queryer(ctx).QueryRowContext(ctx, `SELECT id,edge_cash_session_id,restaurant_id,device_id,shift_id,opened_by_employee_id,closed_by_employee_id,status,opening_cash_amount,closing_cash_amount,opened_at,closed_at,created_at,updated_at FROM cash_sessions WHERE id = ?`, id))
}

func (r *Repository) GetOpenCashSessionByDevice(ctx context.Context, deviceID string) (*domain.CashSession, error) {
	return r.scanCashSession(r.queryer(ctx).QueryRowContext(ctx, `SELECT id,edge_cash_session_id,restaurant_id,device_id,shift_id,opened_by_employee_id,closed_by_employee_id,status,opening_cash_amount,closing_cash_amount,opened_at,closed_at,created_at,updated_at FROM cash_sessions WHERE device_id = ? AND status = 'open'`, deviceID))
}
...
func (r *Repository) scanCashSession(row *sql.Row) (*domain.CashSession, error) {
	...
	err := row.Scan(&v.ID, &v.EdgeCashSessionID, &v.RestaurantID, &v.DeviceID, &v.ShiftID, &v.OpenedByEmployeeID, &closedBy, &status, &v.OpeningCashAmount, &closingCash, &opened, &closedAt, &created, &updated)
	if err != nil {
