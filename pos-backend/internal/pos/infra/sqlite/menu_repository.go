...
func (r *Repository) ListMenuItems(ctx context.Context) ([]domain.MenuItem, error) {
	rows, err := r.queryer(ctx).QueryContext(ctx, `SELECT m.id,m.catalog_item_id,COALESCE(m.category_id,''),COALESCE(m.tag_id,''),COALESCE(c.type,''),m.name,m.price,m.currency,m.tax_profile_id,COALESCE(c.single_unit_per_line,0),COALESCE(m.runtime_status,'available'),m.active,m.created_at,m.updated_at,
...
ORDER BY m.created_at`)
	...
	for rows.Next() {
		...
		var active, singleUnit int
		var stopListActive int
		var created, updated string
		var taxProfileID sql.NullString
		var stopListQuantity sql.NullFloat64
		if err := rows.Scan(&v.ID, &v.CatalogItemID, &v.CategoryID, &v.TagID, &v.ItemType, &v.Name, &v.Price, &v.Currency, &taxProfileID, &singleUnit, &v.RuntimeStatus, &active, &created, &updated, &stopListActive, &stopListQuantity); err != nil {
			return nil, err
		}
...
func (r *Repository) GetMenuItem(ctx context.Context, id string) (*domain.MenuItem, error) {
	...
	var taxProfileID sql.NullString
	var stopListQuantity sql.NullFloat64
	err := r.queryer(ctx).QueryRowContext(ctx, `SELECT m.id,m.catalog_item_id,COALESCE(m.category_id,''),COALESCE(m.tag_id,''),COALESCE(c.type,''),m.name,m.price,m.currency,m.tax_profile_id,COALESCE(c.single_unit_per_line,0),COALESCE(m.runtime_status,'available'),m.active,m.created_at,m.updated_at,
COALESCE(sl.active,0),sl.available_quantity
FROM menu_items m
LEFT JOIN catalog_items c ON c.id = m.catalog_item_id
LEFT JOIN stop_lists sl ON sl.catalog_item_id = m.catalog_item_id AND sl.active = 1 AND sl.cloud_deleted_at IS NULL
WHERE m.id = ?`, id).
		Scan(&v.ID, &v.CatalogItemID, &v.CategoryID, &v.TagID, &v.ItemType, &v.Name, &v.Price, &v.Currency, &taxProfileID, &singleUnit, &v.RuntimeStatus, &active, &created, &updated, &stopListActive, &stopListQuantity)
	if err != nil {
