	...
	err := s.tx.WithinTx(ctx, func(ctx context.Context) error {
		...
		if menuItem.ItemType != string(domain.CatalogItemService) {
			ticket := &domain.KitchenTicket{
				...
				UnitCode:          catalogItem.BaseUnit,
				StationRoutingKey: catalogItem.KitchenType,
				Course:            line.Course,
...
func (s *Service) UpdateOrderLineDetails(ctx context.Context, cmd UpdateOrderLineDetailsCommand) (*domain.OrderLine, error) {
	...
	err := s.tx.WithinTx(ctx, func(ctx context.Context) error {
		...
		if err := s.repo.UpdateKitchenTicketLineDetails(ctx, line.ID, line.Course, line.Comment, shared.DBTime(now)); err != nil {
			return err
		}
...
func (s *Service) CloseOrder(ctx context.Context, cmd CloseOrderCommand) (*domain.Order, error) {
	...
	err := s.tx.WithinTx(ctx, func(ctx context.Context) error {
		...
		if _, err := shared.EnsureOperatorSession(ctx, s.repo, cmd.CommandMeta, string(shared.PermissionOrderClose)); err != nil {
			return err
		}
		var err error
		order, err = s.repo.GetOrder(ctx, cmd.OrderID)
