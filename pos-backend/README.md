...
Проект еще не был запущен в production. Реальных production БД с клиентскими данными нет, поэтому production data migration до первого запуска не требуется. SQLite clean install использует один canonical `migrations/sqlite/001_init.sql`, который сразу создает текущую runtime-схему без `payments.check_id`; runtime version/checksum metadata создается кодом startup framework.
