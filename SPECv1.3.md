...
```text
Этап 0/1: Edge Core Skeleton & Sync Foundation — выполнено
Этап 2: Cash & Shifts Core — выполнено / почти выполнено
Этап 3: Orders, Prechecks & Taxes — в работе
Этап 4: Payments & Final Checks — runtime slice включен
Этап UI-0: pos-ui shell — выполнено
Этап UI-1: POS Terminal Core cashier surface — выполнен первый cashier slice
```

Фактическое состояние текущего кода: публичный runtime `Order -> Precheck -> Payment -> Check` включен. `IssuePrecheck` создает issued precheck и переводит order в `locked`; `CancelPrecheck` доступен публично через manager override; payment capture идет через precheck, требует открытую кассовую смену, поддерживает partial payments и создает final `Check` только после полной оплаты. Публичные compatibility endpoints для старой check/payment модели удалены.

Реализовано сейчас: `auth_sessions` являются технической login/logout-сессией устройства и клиента, а не рабочей сменой. Доменные `shifts` трактуются как личные смены сотрудников и уникальны по открытому `restaurant_id + employee_id`. Без открытой личной смены сотрудника запрещены business/runtime операции, кроме открытия личной смены и чтения последних личных смен текущего actor. Заказы и пречеки требуют личную смену, но не требуют кассовую смену. Оплаты и cash drawer events требуют открытую кассовую смену (`cash_sessions`).

Запланировано далее: данные личной смены сотрудника используются для учета рабочего времени после MVP.

Auth/device состояние: operator/business flows требуют active employee session, matching `actor_employee_id`, `session_id`, `client_device_id` и permissions там, где нужны. System/device flows (`sync`, pairing/status, будущие diagnostics/hardware callbacks) не завязаны на employee session. Lock screen равен backend logout через `POST /api/v1/auth/logout`; session становится `revoked`, новый PIN login создает новую session.
...
```text
/pair  -> production provisioning: Cloud approval polling или license code; legacy POST /api/v1/system/pair остается compatibility path
/login -> POST /api/v1/auth/pin-login
/pos   -> cashier terminal: personal employee shift, cash shift, hall/table, active order, menu, precheck, payment, final check
/lock  -> POST /api/v1/auth/logout
```

Server state в UI идет через TanStack Query. Pinia хранит только локальное состояние identity/session. `client_device_id` генерируется frontend-клиентом через `crypto.randomUUID()` и хранится в `localStorage`; это identity конкретного UI-клиента, а не Edge Node. `node_device_id` не генерируется frontend; он хранится в POS Edge Backend, приходит в UI из provisioning status и обозначает POS Edge Backend.
...
sync
pairing/status
provisioning/register-cloud
provisioning/pair-via-license
diagnostics
hardware callbacks
будущие device operations
```
...
```text
Option A: POS Edge -> Cloud POST /api/v1/devices/register, Cloud admin assign, Edge polling assignment-status, snapshot apply, local pair.
Option B: Cloud generate-pairing-code, License Server resolve, Edge snapshot apply, local pair.
```
...
# 5. DDD-контексты / карта контекстов POS Core

POS Core реализуется как modular monolith с явными bounded contexts. Запрещено создавать один общий `POSContext`, который владеет всеми сущностями.

Подробная таблица владения зафиксирована в `docs/architecture/DDD-CONTEXT-MAP.md`. Этот раздел фиксирует архитектурные инварианты, которые обязательны для кода и профильных спецификаций.

| Контекст | Владение доменом | Реализовано сейчас | Запланировано далее | Вне текущего объема | Запрещенные зависимости |
| --- | --- | --- | --- | --- | --- |
| `Organization` | `restaurants`, `devices`, `edge_node_identity`, `client_devices`, `employees`, `roles`, org/restaurant settings | restaurants/devices/client_devices/employees/roles/auth pairing foundation | restaurant settings, timezone/day-start policy, workstation/cash zone modeling при необходимости | enterprise org tree, multi-brand/legal entity complexity | не владеет order/payment/shift runtime |
| `Catalog` | `catalog_items`, menu-visible item identity, units, SKU/barcode/quick code, recipes reference | `catalog_items`, упрощенные `menu_items`, `recipe_versions` / `recipe_lines` foundation | modifier model, POS category, preparation/sales place mapping, unit normalization | allergens/nutrition/media-rich catalog без явного pilot requirement | не владеет pricing decisions |
| `Pricing` | price lists, channel/location pricing, currency/rounding policy, tax-in-price policy | simplified `menu_items.price` и money/currency invariants | явный `PriceList` / `PriceListItem` или документированная MVP-граница цены | happy hours, dynamic pricing, advanced promotions | не владеет `Catalog` identity и не меняет payment facts |
| `Order` | `orders`, `order_lines`, order state, guest count, table reference, precheck issue intent | `Order -> Precheck` runtime, order editing, lock/close lifecycle | modifiers/order line modifiers, void reasons, split/merge/transfer decisions при необходимости | complex guest ownership, courses | не пишет напрямую в `Inventory`, не владеет payment facts |
| `Payment` | `payments`, `payment_attempts`, payment methods, payment allocation, refunds/tips далее | cash/card/other, trusted manual card, partial payments, immutable captured payment | explicit payment allocation docs, refund policy decision | real PSP integration | не мутирует `Order` / `Check` напрямую, кроме orchestration через application service |
| `Fiscal / Tax` | tax policy, fiscal/legal receipt mapping, fiscal device integration далее | line/check totals, rounding policy foundation и immutable snapshots для reprint | tax profile entity и fiscal receipt/check mapping | real fiscalization integration без отдельного pilot requirement | не владеет PSP, order lifecycle и accounting |
| `Production` | kitchen tickets, KDS stations, cooking statuses, fire/hold, prep queues | только architecture/foundation references | event contract from `Order` to `Production`, если KDS входит в пилот | KDS runtime, если roadmap держит его после пилота | не списывает inventory напрямую |
| `Inventory` | `stock_documents`, `stock_moves`, `stock_balances`, `item_costs`, recipe consumption | schema/domain foundation for recipes, purchase receipts, stock docs/moves/balances/item costs | stock movement app services и consumption policy при необходимости пилота | complex AVCO/FIFO/batch costing | меняется только через stock documents / stock moves |
| `Procurement` | suppliers, supplier orders, purchase receipts, supplier prices | `purchase_receipts` / `purchase_receipt_lines` foundation | решить, является ли purchase receipt частью `Inventory` foundation или отдельным контекстом `Procurement` | full supplier workflow/autopurchase | не владеет stock balances и costing |
| `CRM` | customer profile, contacts, preferences, history | не реализовано, кроме `guest_count` на order | `customer_id` reference при необходимости | full CRM | не владеет order lifecycle |
| `Loyalty` | bonuses, coupons, promos, discount decisioning | не реализовано | discount result contract при необходимости | full loyalty engine | frontend не рассчитывает authoritative financial totals |
| `Delivery / Channel` | sales channel, delivery status, aggregators, commissions | basic dine-in/table flow | dining mode/channel enum при необходимости | aggregators/delivery logistics | не владеет payment facts |
| `Reservation / Table` | halls, tables, reservations, occupancy | halls/tables | решить, остаются ли halls/tables как Floor subcontext внутри `Reservation / Table` для MVP | reservations/waitlist без pilot requirement | не владеет order lifecycle после создания order |
| `Staff / Shift` | shifts, `cash_sessions`, `cash_drawer_events`, manager overrides, operational audit actor context | personal shifts, cash sessions, cash drawer events, manager override audit | `business_date_local` propagation и attendance semantics | full HR/timekeeping | `auth_session != employee shift != cash_session` |
| `Accounting / Finance` | reconciliation, P&L, revenue/cost/profit reporting, cash book | basic Cloud projections / shift finance foundation | reporting projections over operational events | full accounting ERP | не меняет runtime payment/order facts |
| `Event / Integration` | `local_event_log`, `pos_sync_outbox`, `SyncEnvelope`, Cloud sync receiver, master-data provisioning/import | local event log, outbox, sync envelope, Edge -> Cloud, Cloud -> Edge master-data provisioning for supported streams, item-level ACK batch flow | projection query endpoints, production auth perimeter for provisioning | marketplace/plugin system | sync transports events and master-data changes; sync не содержит domain business logic |
...
Cloud-owned master data не мутируется Edge runtime app services.
Edge-owned operational data синхронизируется в Cloud через append/event flow.
```

Текущее соответствие коду:

- `pos-backend/internal/pos/domain` уже разбит на пакеты `restaurant`, `device`, `employee`, `floor`, `catalog`, `menu`, `shift`, `order`, `precheck`, `check`, `cash`, `shared`, `inventory`.
- `domain/aliases.go` является transitional package API/facade над domain packages, а не bounded context.
- `app/service.go` агрегирует use case services и выполняет application-layer orchestration.
...
2. После неудачной печати UI обязан показать оператору статус ошибки и действие повторной печати.
3. Backend обязан повторно сформировать печатный payload из сохраненного snapshot, а не пересчитывать суммы заново.
4. Реализовано сейчас: controlled reprint для precheck/check выполняется из immutable snapshots и пишет audit/outbox events `PrecheckReprinted` / `CheckReprinted`.
...
Реализовано сейчас: Cloud не возвращает `all-or-nothing` для batch endpoint. Для каждого события в батче Cloud возвращает индивидуальный статус:

- `accepted`: Edge ставит `status = 'sent'`.
- `rejected`: Edge переводит строку в `suspended`.
- `retryable`: Edge увеличивает `attempts`, рассчитывает `next_retry_at` и возвращает запись в `pending` или `failed` по лимиту попыток.
  ...
  "items": [
    {
      ...
      "error_message": "optional_safe_message",
      "ack": {}
    }
  ]
}
...
|---|---|---|---|---|---|
| Order | `open` | `IssuePrecheck` | `locked` | cashier | active personal employee shift, snapshot created, outbox written |
...
## 11.1 Personal Employee Shift

Personal Employee Shift — личная смена сотрудника. В текущей canonical SQLite схеме она хранится в `shifts`.

Инварианты:

```text
у сотрудника может быть только одна active personal employee shift в ресторане
нельзя создать order без active personal employee shift
нельзя закрыть personal employee shift с open/locked orders
нельзя закрыть personal employee shift с active cash shift
```
...
Cash Shift — кассовая смена устройства. В текущей canonical SQLite схеме она хранится в `cash_sessions`.

Инварианты:

```text
нельзя открыть cash shift без active personal employee shift
нельзя иметь две active cash shifts на device/cash drawer
cash drawer events требуют active cash shift
payment capture требует active cash shift
```
...
POST /api/v1/employee-shifts/{shift_id}/close
POST /api/v1/cash-shifts/open
POST /api/v1/cash-shifts/{cash_shift_id}/close
...
Order ссылается на `table_id` и хранит `table_name` как snapshot для чтения. Floor plan остается отдельной будущей foundation; текущий runtime покрывает halls + tables.
...
```text
нельзя создать order без active personal employee shift
нельзя issue precheck без active personal employee shift
нельзя capture payment без active cash shift
...
## Этап 0 и 1 — Edge Core Skeleton & Cloud Sync
...
## Этап 2 — Cash & Shifts Core

Статус: выполнено / почти выполнено.
...
## Этап 5 — Catalog, Recipes & Menu Updates
...
## Этап 11 — Real PSP Integrations & Evidence Archive
...
Обязательные тесты для этапов 3/4:
