...
- POS Edge backend имеет безопасный lifecycle surface локальной SQLite БД: `GET /api/v1/storage/status` и `POST /api/v1/storage/retention/dry-run` считают объемы закрытых заказов, финансового ledger и outbox/sync состояния без удаления данных; `POST /api/v1/storage/archive/export-plan` возвращает manifest-only план кандидатов и protected flags; `POST /api/v1/storage/archive/export` создает export-only JSONL archive и manifest для closed orders до cutoff. Физическое удаление, destructive apply, restore/read из архива, `VACUUM` и compaction не реализованы сейчас.
