...
- `GET /api/v1/orders/closed` реализовано сейчас как bounded read: default `limit=50`, max `limit=100`, `offset`, фильтры `business_date_local`, `from_business_date_local`, `to_business_date_local`, `shift_id`, `device_id`, `check_id`, stable newest-first sort. POS UI использует бизнес-дату и постраничную навигацию; retention/archive/compaction больших локальных БД запланированы далее, но не реализованы сейчас.
