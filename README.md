...
- POS Edge backend имеет безопасный lifecycle surface локальной SQLite БД: `GET /api/v1/storage/status` и `POST /api/v1/storage/retention/dry-run` считают объемы закрытых заказов, финансового ledger, active/open blockers и outbox/sync состояния без удаления данных; `POST /api/v1/storage/archive/export-plan` возвращает manifest-only план кандидатов, protected flags и `result_mode = plan_only`; `POST /api/v1/storage/archive/export` создает export-only JSONL archive и manifest для closed orders до cutoff с `runtime_rows_deleted = false`; `POST /api/v1/storage/archive/read-plan` non-destructive проверяет manifest/version/SHA/counts/snapshot payload; `POST /api/v1/storage/archive/lookup` streaming-способом возвращает immutable check/precheck snapshot preview по `check_id` или `order_id` без восстановления в SQLite; `POST /api/v1/storage/archive/apply-plan` проверяет archive/runtime blockers, но всегда возвращает `result_mode = apply_blocked`. Физическое удаление, destructive apply, restore в active SQLite, `VACUUM` и compaction не реализованы сейчас.
